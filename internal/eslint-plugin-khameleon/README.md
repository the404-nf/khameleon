# @khameleon/eslint-plugin

ESLint plugin for Khameleon design system token enforcement.
...
| **Strict**      | Agents/CI | Errors (fail build) | `CI=true` or `KHAMELEON_STRICT_LINT=1` |
...
### `@khameleon/no-hardcoded-styles`

Detects hardcoded CSS values in `stylex.create()` that should use Khameleon tokens:
...
### `@khameleon/require-letter-spacing`
...
pnpm lint:strict
# or
KHAMELEON_STRICT_LINT=1 pnpm lint
...
```
  12:15  error  Use textSizeVars token instead of hardcoded fontSize  @khameleon/no-hardcoded-styles
  17:16  error  Use fontWeightVars token instead of hardcoded fontWeight  @khameleon/no-hardcoded-styles
  22:12  error  Use colorVars token instead of hardcoded color  @khameleon/no-hardcoded-styles
  ...
```
...
{
  files: ["packages/core/src/**/*.{ts,tsx}"],
  plugins: { '@khameleon': khameleonPlugin },
  rules: {
...
### `@khameleon/presentational-component`
