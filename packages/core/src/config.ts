...
/**
 * @file config.ts
 * @input An app's Khameleon config object.
 * @output A type-preserving `createConfig` helper plus the `KhameleonConfig`
 *   surface authors write in `khameleon.config.{ts,mjs,js}`.
 * @position Consumer-facing authoring surface. Lives in `@khameleon/core`
 ...
 *   `@khameleon/cli/config` for back-compat.
 *
...
/** User config exported from khameleon.config.{ts,mjs,js}. */
export interface KhameleonConfig {
...
/**
 * Type-preserving helper for the Khameleon config file. Returns its argument
 ...
 */
export function createConfig<T extends KhameleonConfig>(config: T): T {
  return config;
}
