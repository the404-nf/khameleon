...
export interface PropDoc {
  ...
  required?: boolean;
  /** For ReactNode props: the Khameleon components this slot typically accepts.
 ...
 * @example
 * ```
 * {className: 'khameleon-button', visualProps: ['variant', 'size']}
 * {className: 'khameleon-avatar-status-dot', visualProps: ['variant']}
 * {className: 'khameleon-card'}
 * ```
 */
export interface ThemingTarget {
  /** The stable CSS class name rendered by the component.
   *  Always starts with `khameleon-`.
   *  e.g. `"khameleon-button"`, `"khameleon-avatar-status-dot"`, `"khameleon-card"` */
  className: string;
...
export interface ComponentVar {
   ...
   * `khameleon theme build` errors if a theme sets a private var directly.
   */
  private?: boolean;
}
...
export interface ComponentEntry {
  /** Full export name including Khameleon prefix. e.g. `"TableRow"`,
   ...
   *  (e.g. `"TableRow"` → `"Khameleon Table Row"`). See `BaseDoc.displayName`. */
  displayName: string;
...
export interface PlaygroundConfig {
   ...
   */
  wrapper?: {
    /** Parent component name as exported from `@khameleon/core`, e.g. `'TabList'`. */
    component: string;
 ...
 */
interface BaseDoc {
  /** Directory name without the Khameleon prefix, PascalCase.
   ...
   *  Lowercase only. Used by `khameleon component <term>` for fuzzy matching.
   ...
   *  directory's doc covers a group but some Khameleon*.tsx files shouldn't
...
export interface ComponentRef {
  /** Full export name including Khameleon prefix, e.g. `"ChatComposer"`. Must
 ...
 * and can be individually retrieved via `khameleon docs <topic> <section>`.
 */
 ...
 * in the docs/ directory and it shows up in `khameleon docs`.
 *
 ...
 *
 *   /** @type {import('@khameleon/core').TemplateDoc} *\/
 *   export const doc = { ... };
 *
...
interface BaseTemplateDoc {
   ...
   *  `khameleon template <name>` — it's only hidden from the browsable gallery.
 ...
 * they cluster related components but have no corresponding Khameleon*.tsx file.
...
export interface HookDoc {
   ...
   *  Enables cross-referencing: \`khameleon component Toast\` can mention useToast,
   *  and \`khameleon hook useToast\` can link back to Toast. */
  relatedComponents?: string[];
  /** Other hook names this hook is commonly used with. */
  relatedHooks?: string[];
  /** Import path, e.g. '@khameleon/core/hooks' or '@khameleon/core/Toast'. */
  importPath?: string;
