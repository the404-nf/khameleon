...
describe('mergeProps', () => {
  describe('string-first form (xdsClass, stylexResult, className?, style?)', () => {
    it('returns the xds class alone when the stylex result has no class', () => {
      expect(mergeProps('khameleon-button', {className: ''})).toEqual({
    ...
    it('prefixes the xds class before the stylex class name', () => {
      expect(mergeProps('khameleon-button', {className: 'x1 x2'})).toEqual({
        className: 'khameleon-button x1 x2',
        style: undefined,
      });
    });
    ...
    it('appends a consumer className after the stylex classes', () => {
      expect(
        mergeProps('khameleon-button', {className: 'x1'}, 'consumer'),
      ).toEqual({
        className: 'khameleon-button x1 consumer',
        style: undefined,
      });
    });

    it('omits the consumer className when it is falsy', () => {
      expect(mergeProps('khameleon-button', {className: 'x1'}, undefined)).toEqual(
        {
          className: 'khameleon-button x1',
          style: undefined,
        },
      );
      expect(mergeProps('khameleon-button', {className: 'x1'}, '')).toEqual({
        className: 'khameleon-button x1',
        style: undefined,
      });
    });
    ...
    it('defaults the stylex result to an empty class when omitted', () => {
      expect(mergeProps('khameleon-button')).toEqual({
        className: 'khameleon-button',
        style: undefined,
      });
    });
    ...
    it('uses only the stylex style when no consumer style is given', () => {
      const result = mergeProps('khameleon-button', {
        className: 'x1',
        style: {color: 'red'},
      });
    ...
    it('uses only the consumer style when the stylex result has none', () => {
      const result = mergeProps('khameleon-button', {className: 'x1'}, undefined, {
        color: 'blue',
      });
