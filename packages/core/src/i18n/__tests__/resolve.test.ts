...
describe('resolve — basic lookup', () => {
  ...
  test('returns the key itself and warns for unknown key', () => {
    ...
    const out = resolve(
      '@khameleon.does.not.exist',
      undefined,
      'en',
      {},
      undefined,
    );
    expect(out).toBe('@khameleon.does.not.exist');
...
describe('resolve — provider messages', () => {
  const catalog: Catalog = {
    '@khameleon.pagination.next': {defaultMessage: 'Suivant'},
  };
...
describe('resolve — locale-chain fallback (regional → language)', () => {
  const messages: MessagesByLocale = {
    'pt-BR': {
      '@khameleon.pagination.next': {defaultMessage: 'Próxima (BR)'},
    },
    pt: {
      '@khameleon.pagination.next': {defaultMessage: 'Próxima'},
      '@khameleon.pagination.previous': {defaultMessage: 'Anterior'},
    },
  };
...
describe('resolve — overrides', () => {
  test('per-locale override wins over shipped catalog', () => {
    const messages: MessagesByLocale = {
      fr: {'@khameleon.pagination.next': {defaultMessage: 'Suivant'}},
    };
    const overrides: Overrides = {
      fr: {'@khameleon.pagination.next': 'Suiv.'},
    };
    const out = resolve(
      '@khameleon.pagination.next',
      undefined,
  ...
  test('overrides respect the locale chain (pt-BR → pt)', () => {
    ...
    const overrides: Overrides = {
      pt: {'@khameleon.pagination.next': 'Próxima (pt override)'},
    };
    const out = resolve(
      '@khameleon.pagination.next',
      undefined,
  ...
  test('override for a different locale does not apply', () => {
    const overrides: Overrides = {
      es: {'@khameleon.pagination.next': 'Siguiente'},
    };
    const out = resolve(
      '@khameleon.pagination.next',
      undefined,
...
describe('resolve — warn-once behavior', () => {
  test('duplicate missing-key warnings are suppressed', () => {
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
    resolve('@khameleon.does.not.exist', undefined, 'en', {}, undefined);
    resolve('@khameleon.does.not.exist', undefined, 'en', {}, undefined);
    resolve('@khameleon.does.not.exist', undefined, 'en', {}, undefined);
