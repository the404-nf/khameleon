 ...
 *   1. Khameleon's default operators (built via `usePowerSearchConfig`) look
  ...
  test('provider overrides translate default operator labels', () => {
    ...
    render(
      <InternationalizationProvider
        locale="fr"
        overrides={{
          fr: {'@khameleon.powersearch.operator.contains': 'contient'},
        }}>
        <Harness fieldDefs={[nameField]} filters={[filter]} />
      </InternationalizationProvider>,
    );
  ...
  test('resultCount ICU plural swaps under provider overrides', () => {
    render(
      <InternationalizationProvider
        ...
        overrides={{
          fr: {
            '@khameleon.powersearch.resultCount':
