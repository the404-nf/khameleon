 ...
 * @position Provider component for khameleon i18n locale + messages
 *
 * Wraps a subtree with a locale and (optional) additional message catalogs +
 * overrides. Khameleon components inside the subtree resolve their strings via
 * this context. If a consumer never renders a provider, khameleon components
...
export interface InternationalizationProviderProps {
   ...
   * `en` is bundled with khameleon and never needs to be listed here.
   *
   * @example
   * ```
   * import {fr} from '@khameleon/core/locales/fr.json';
   ...
   *   locale="fr"
   *   overrides={{fr: {'@khameleon.pagination.next': 'Suivant'}}}>
   * ```
   */
  overrides?: Overrides;
  children: ReactNode;
...
/**
 * Provides locale + additional messages + overrides to all khameleon
