...
describe('generateThemeRules', () => {
  ...
  it('includes .khameleon-heading.level-* rules for all 6 levels', () => {
    for (let level = 1; level <= 6; level++) {
      const rule = rules.find(r =>
        r.includes(`.khameleon-heading.level-${level}`),
      );
  ...
  it('includes .khameleon-text.* rules for all 5 types', () => {
    for (const type of ['body', 'large', 'label', 'code', 'supporting']) {
      const rule = rules.find(r => r.includes(`.khameleon-text.${type}`));
      expect(rule).toBeDefined();
  ...
  it('includes explicit component overrides', () => {
    const buttonRule = rules.find(r => r.includes('.khameleon-button.secondary'));
  ...
  it('applies pseudo-class suffixes to both compat selector prefixes', () => {
    ...
    expect(
      pseudoRules.some(rule => rule.includes('.khameleon-button:hover')),
    ).toBe(true);
  });
  ...
  it('includes prose heading rules with computed values', () => {
    ...
    // via StyleX (@layer khameleon-base). Emitting margins here would re-introduce
  ...
  it('includes color prop overrides for text and heading', () => {
    expect(rules.some(r => r.includes('.khameleon-text.primary'))).toBe(true);
    expect(rules.some(r => r.includes('.khameleon-text.secondary'))).toBe(true);
    expect(rules.some(r => r.includes('.khameleon-heading.primary'))).toBe(true);
    expect(rules.some(r => r.includes('.khameleon-heading.disabled'))).toBe(true);
    expect(rules.some(r => r.includes('.khameleon-text.active'))).toBe(false);
    expect(rules.some(r => r.includes('.khameleon-text.accent'))).toBe(true);
  });
  ...
  it('generateThemeCSS returns prose and component blocks with @scope', () => {
    ...
    expect(combined).toContain('@scope ([data-khameleon-theme="default"])');
    expect(combined).toContain('to ([data-khameleon-theme])');
...
describe('derived var expansion', () => {
  it('emits borderRadius AND internal var for card', () => {
    ...
    const rules = generateThemeRules(theme);
    const cardRule = rules.find(r => r.includes('.khameleon-card'));
  ...
  it('emits borderRadius AND internal var for dropdown-menu', () => {
    ...
    const rules = generateThemeRules(theme);
    const rule = rules.find(r => r.includes('.khameleon-dropdown-menu'));
    expect(rule).toBeDefined();
  ...
  it('emits internal vars for chat composer', () => {
    ...
    const rules = generateThemeRules(theme);
    const rule = rules.find(r => r.includes('.khameleon-chat'));
    expect(rule).toBeDefined();
  ...
  it('emits internal var for button borderRadius', () => {
    ...
    const rules = generateThemeRules(theme);
    const rule = rules.find(r => r.includes('.khameleon-button'));
    expect(rule).toBeDefined();
  ...
  it('does not emit derived vars for components without registry entries', () => {
    ...
    const rules = generateThemeRules(theme);
    const rule = rules.find(r => r.includes('.khameleon-badge'));
    expect(rule).toBeDefined();
  ...
  it('container expansion still works for card padding', () => {
    ...
    const rules = generateThemeRules(theme);
    const rule = rules.find(r => r.includes('.khameleon-card'));
    expect(rule).toBeDefined();
    // Container expansion emits --khameleon-card-padding token
    expect(rule).toContain('--khameleon-card-padding: 20px');
  });
  ...
  it('handles variant-specific derived vars', () => {
    ...
    const rules = generateThemeRules(theme);
    const rule = rules.find(r => r.includes('.khameleon-card.muted'));
    expect(rule).toBeDefined();
    expect(rule).toContain('border-radius: 16px');
...
describe('brutalist-style derived expansion', () => {
  ...
  it('card padding emits container tokens via derived expansion', () => {
    ...
    const rule = rules.find(r => r.includes('.khameleon-card'));
    expect(rule).toBeDefined();
    expect(rule).toContain('--khameleon-card-padding: 24px');
  });
