...
describe('generateOnMediaCSS', () => {
  it('emits @scope with [data-khameleon-media] token rules', () => {
    const theme = defineTheme({name: 'test'});
    const css = generateOnMediaCSS(theme);
    expect(css).toContain(
      '@scope ([data-khameleon-theme="test"])',
    );
    // Same scope boundary as main theme
    expect(css).toContain('to ([data-khameleon-theme])');
    expect(css).toContain('[data-khameleon-media="dark"]');
  ...
  it('emits light media rules', () => {
    const theme = defineTheme({name: 'test'});
    const css = generateOnMediaCSS(theme);
    expect(css).toContain('[data-khameleon-media="light"]');
  ...
  it('emits component override rules', () => {
    ...
    const css = generateOnMediaCSS(theme);
    expect(css).toContain(
      ':is([data-khameleon-media="dark"]) :is(.khameleon-button.secondary)',
    );
    expect(css).toContain(
  ...
  it('emits pseudo-class rules for on-media components', () => {
    ...
    const css = generateOnMediaCSS(theme);
    expect(css).toContain(
      ':is([data-khameleon-media="dark"]) :is(.khameleon-button):hover',
    );
...
describe('reset.css baseline media rules', () => {
   ...
   * MediaTheme: a color-scheme flip on [data-khameleon-media].
   *
  ...
  it('flips color-scheme on [data-khameleon-media="dark"]', () => {
    const darkMatch = resetCSS.match(
      /:where\(\[data-khameleon-media="dark"\]\)\s*\{([^}]+)\}/,
    );
    expect(darkMatch).not.toBeNull();
    expect(darkMatch![1]).toContain('color-scheme: dark');
  });

  it('flips color-scheme on [data-khameleon-media="light"]', () => {
    const lightMatch = resetCSS.match(
      /:where\(\[data-khameleon-media="light"\]\)\s*\{([^}]+)\}/,
    );
