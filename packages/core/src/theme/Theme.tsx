 ...
 * - Token overrides set as CSS custom properties on [data-khameleon-theme]
 * - Component overrides scoped via @scope'd CSS selectors on the stable Khameleon
 ...
 * - `data-khameleon-theme` — enables @scope'd theme CSS to reach elements rendered
 ...
 * Built themes (from `khameleon theme build`) skip injection — their CSS
...
function useThemeStyleInjection(theme: DefinedTheme): void {
  ...
  useInsertionEffect(() => {
    ...
    const themeKey = `khameleon-theme-${theme.name}`;
    ...
    if (!warnedThemes.has(theme.name)) {
      ...
      console.warn(
        `[Khameleon] Theme "${theme.name}" is using runtime style injection. ` +
          `For better performance, use the pre-built theme:\n\n` +
          `  import {${theme.name}Theme} from '@khameleon/theme-${theme.name}/built';\n` +
          `  import '@khameleon/theme-${theme.name}/theme.css';\n\n` +
          `For custom themes, run \`npx khameleon theme build <file>\` to generate ` +
          `the built artifacts.`,
      );
    }
    ...
    // Component overrides go into @layer khameleon-theme — above StyleX layers
    ...
    if (component) {
      ...
      compStyle.textContent = `@layer khameleon-theme {\n${component}\n}`;
    ...
    return () => {
      ...
      // khameleon id marker, which is always written above).
 ...
 * - `data-khameleon-theme` (theme name) — enables @scope'd theme CSS to reach elements
 ...
 * Theme provider component
 *
 * Sets data-khameleon-theme attribute so @scope'd CSS takes effect.
 ...
 * it syncs `data-theme` and `data-khameleon-theme` to `<html>` so browser
    ...
    <ThemeContext value={ctxValue}>
      <ThemeNestingContext value={true}>
        <div
          ...
          data-khameleon-theme={theme.name}
