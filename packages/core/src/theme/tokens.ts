 ...
 *
 * Use these helpers when code outside React hooks needs Khameleon token values:
...
/**
 * Return a CSS custom property reference for an Khameleon token name.
 *
 ...
 * stay connected to the active Khameleon theme through the CSS cascade.
 *
 * @example
 * ```ts
 * const theme = {
...
/** Flat map of every known Khameleon token name to its `var(--token-name)` reference. */
...
/**
 * Resolve all Khameleon token values for a theme and effective color mode.
 *
...
/** Resolve one Khameleon token value for a theme and effective color mode. */
