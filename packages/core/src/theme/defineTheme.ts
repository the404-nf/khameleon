...
/**
 * defineTheme — Create an Khameleon theme from a flat token map.
 *
 * Two distribution modes:
 * - Unbuilt: Theme generates CSS and injects a <style> tag at runtime
 * - Built: `npx khameleon theme build` pre-compiles to a CSS file; Theme just
 *   sets the data-khameleon-theme attribute
 *
 * Token values can be:
...
/** All valid Khameleon core token names */
...
/** All valid Khameleon token names — core + domain tokens */
...
export interface DefineThemeInput {
  /** Theme name — used for data-khameleon-theme attribute and identification */
  name: string;
   ...
   * the theme's data-khameleon-theme attribute via @scope.
   *
   * Use `prop:value` keys to target specific visual props. New values
   * not in the base type are automatically detected by `khameleon theme build`
...
/** All Khameleon token defaults as a flat map. Useful for resolving full token sets. */
...
/**
 * Create an Khameleon theme.
 *
 * Pass only the tokens you want to override — everything else
 * inherits from the Khameleon defaults.
 *
