...
describe('generateThemeCSS', () => {
  ...
  it('splits prose into reset layer and components into khameleon-theme', () => {
    const theme = defineTheme({
      name: 'split-test',
      components: {
    ...
    expect(prose).not.toContain('.khameleon-button');
    // Component block should contain overrides
    expect(component).toContain('.khameleon-button');
...
describe('generateThemeCSS with components', () => {
  it('generates scoped CSS for base component overrides', () => {
    ...
    const css = generateThemeCSSFlat(theme);
    expect(css).toContain('.khameleon-card {');
    expect(css).toContain('border-width: 2px');
    expect(css).toContain('border-color: var(--color-accent)');
    expect(css).toContain('.khameleon-button {');
  ...
  it('generates variant selectors from prop:value keys', () => {
    ...
    const css = generateThemeCSSFlat(theme);
    expect(css).toContain('.khameleon-button.secondary');
  ...
  it('generates compound selectors from prop:value+prop:value keys', () => {
    ...
    const css = generateThemeCSSFlat(theme);
    expect(css).toContain('.khameleon-button.destructive.sm');
...
describe('custom status via components', () => {
  ...
  it('generates correct CSS selectors for banner status extensions', () => {
    ...
    const css = generateThemeCSSFlat(theme);
    // parseStyleKey('status:neutral') → '.neutral', so CSS should have .khameleon-banner.neutral
    expect(css).toContain('.khameleon-banner.neutral');
  ...
  it('generates correct CSS for custom button variant', () => {
    ...
    const css = generateThemeCSSFlat(theme);
    expect(css).toContain('.khameleon-button.primary-muted');
...
describe('pseudo-class overrides in components', () => {
  it('generates pseudo-class rules from nested objects', () => {
    ...
    const css = generateThemeCSSFlat(theme);
    // Base rule
    expect(css).toContain('.khameleon-radio {');
    expect(css).toContain('border-color: #8F9296');
    // Pseudo rule — separate selector
    expect(css).toContain('.khameleon-radio:hover {');
  ...
  it('generates pseudo-class rules on variant selectors', () => {
    ...
    expect(css).toContain('.khameleon-button.primary-muted {');
    expect(css).toContain('background-color: #ECF5FF');
    expect(css).toContain('.khameleon-button.primary-muted:hover {');
    expect(css).toContain('background-color: #D6EBFF');
    expect(css).toContain('.khameleon-button.primary-muted:focus-visible {');
  ...
  it('handles pseudo-only overrides (no base properties)', () => {
    ...
    expect(css).not.toMatch(/\.khameleon-switch\s*\{\s*\}/);
    // Should emit the pseudo rule
    expect(css).toContain('.khameleon-switch:hover {');
  });
  ...
  it('keeps non-pseudo string values as regular properties', () => {
    ...
    expect(css).toContain('border-color: var(--color-accent)');
    // No pseudo rules
    expect(css).not.toContain('.khameleon-card:');
  });
});
...
describe('container padding mapping', () => {
  it('maps uniform padding to shorthand token on card', () => {
    ...
    expect(css).toContain('--khameleon-card-padding: 20px');
    // Should NOT emit directional tokens (shorthand covers all sides)
    expect(css).not.toContain('--khameleon-card-padding-inline');
    expect(css).not.toContain('--khameleon-card-padding-block-start');
  });
  ...
  it('maps asymmetric padding to directional tokens', () => {
    ...
    const css = generateThemeCSSFlat(theme);
    expect(css).toContain('--khameleon-card-padding-inline: 20px');
    expect(css).toContain('--khameleon-card-padding-block-start: 16px');
    expect(css).toContain('--khameleon-card-padding-block-end: 16px');
    // Should NOT emit shorthand (sides differ)
    expect(css).not.toMatch(/--khameleon-card-padding:[^-]/);
  });
  ...
  it('maps paddingBlock and paddingInline separately', () => {
    ...
    const css = generateThemeCSSFlat(theme);
    expect(css).toContain('--khameleon-card-padding-inline: 16px');
    expect(css).toContain('--khameleon-card-padding-block-start: 24px');
    expect(css).toContain('--khameleon-card-padding-block-end: 24px');
  });
  ...
  it('works for section and dialog too', () => {
    ...
    expect(css).toContain('--khameleon-section-padding: 12px');
    // Dialog — asymmetric → directional
    expect(css).toContain('--khameleon-dialog-padding-inline: 32px');
    expect(css).toContain('--khameleon-dialog-padding-block-start: 24px');
  });
  ...
  it('does NOT map padding on non-container components', () => {
    ...
    expect(css).not.toContain('--khameleon-button-padding');
  });
  ...
  it('maps 3-value padding shorthand', () => {
    ...
    const css = generateThemeCSSFlat(theme);
    expect(css).toContain('--khameleon-card-padding-block-start: 16px');
    expect(css).toContain('--khameleon-card-padding-block-end: 12px');
    expect(css).toContain('--khameleon-card-padding-inline: 20px');
  });
  ...
  it('maps paddingInline shorthand with two values', () => {
    ...
    const css = generateThemeCSSFlat(theme);
    expect(css).toContain('--khameleon-card-padding-inline-start: 12px');
    expect(css).toContain('--khameleon-card-padding-inline-end: 20px');
  });
  ...
  it('maps paddingBlockStart alone', () => {
    ...
    const css = generateThemeCSSFlat(theme);
    expect(css).toContain('--khameleon-card-padding-block-start: 32px');
    expect(css).not.toContain('--khameleon-card-padding-block-end');
    expect(css).not.toContain('--khameleon-card-padding-inline');
  });
  ...
  it('maps paddingInlineStart and paddingInlineEnd separately', () => {
    ...
    const css = generateThemeCSSFlat(theme);
    expect(css).toContain('--khameleon-card-padding-inline-start: 8px');
    expect(css).toContain('--khameleon-card-padding-inline-end: 24px');
  });
});
