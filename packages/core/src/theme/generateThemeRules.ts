 ...
 * path (`khameleon theme build` pre-compiles to CSS files).
 *
 ...
 * (`data-khameleon-theme`).
 *
 * @example themeScopeStart('mytheme')
 *   -> '[data-khameleon-theme="mytheme"]'
 */
...
/** Media-surface selector, e.g. for [data-khameleon-media="dark"]. */
function mediaSelector(surface: string): string {
  return `[data-${dataAttrNamespace}-media="${surface}"]`;
}

/** Component base-class selector, e.g. '.khameleon-button'. */
...
export interface ThemeRulesSplit {
  /** Token overrides + component .khameleon-* overrides + prop-level color rules */
  component: string[];
...
export interface ThemeCSSOutput {
  ...
  prose: string;
  /**
   * Token overrides + component .khameleon-* overrides scoped to the theme.
   * Should be injected into @layer khameleon-theme — above StyleX layers so
 ...
 *
 * Emits the rebranded --khameleon-<component>-padding tokens (shorthand +
 * directional overrides), e.g.:
 *   --khameleon-card-padding: 20px
 *   --khameleon-card-padding-inline: 20px
 *   --khameleon-card-padding-block-start: 20px
 *   --khameleon-card-padding-block-end: 20px
 *
 * The component reads these with an inverted fallback chain
 * (var(--khameleon-*, default)). The container.stylex.ts default styles read
 ...
 * the runtime path (useInsertionEffect) and the build path (khameleon theme build).
 */
export function generateThemeRules(theme: DefinedTheme): string[] {
  ...
  // 2. Component overrides (.khameleon-* class rules; components also emit
...
/**
 * Generate component override rules using the .khameleon-* class selector
 ...
 * any class-based style (StyleX, .khameleon-* overrides) should beat.
 * The caller places these in the reset layer (not khameleon-theme) so they
 ...
 *
 * Component rules (tokens, .khameleon-* overrides) are intentional theme overrides
 * that need to beat StyleX — they stay in khameleon-theme (above StyleX layers).
 */
 ...
 * [data-khameleon-media] elements. Parent theme component overrides flow through
 ...
 * - `component`: @scope'd token + .khameleon-* overrides → inject into @layer khameleon-theme
 *
...
export function generateThemeCSS(theme: DefinedTheme): ThemeCSSOutput {
  ...
  // on-media rules in unbounded scope (can reach [data-khameleon-media] elements)
  let componentCss = '';
