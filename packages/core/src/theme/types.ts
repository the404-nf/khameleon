...
/**
 * Khameleon Type Definitions
 *
 * Shared types used across Khameleon components.
 */
 ...
 * or an @import in CSS). Khameleon does not handle font loading — it only
 ...
 * treatment from theme CSS (`.khameleon-text.<custom-type> { ... }`).
 *
 * To add type-safe custom types, use module augmentation:
 * ```ts
 * declare module '@khameleon/core/theme' {
 ...
 * ```
 *
 * `khameleon theme build` generates these augmentations automatically when it
