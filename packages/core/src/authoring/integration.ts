 ...
 *   `KhameleonIntegration` manifest surface.
 ...
 * (khameleon.integration.{ts,mjs,js}) sibling to the integration package's
 * package.json. Identity (name/version) comes from the package's
 * package.json, not from the manifest.
 */
export interface KhameleonIntegration {
...
/**
 * Type-preserving helper for an Khameleon integration manifest. Returns its
 * argument unchanged; the value is the exported TypeScript surface. Validation
 * happens at the CLI load boundary.
 */
export function createIntegration<T extends KhameleonIntegration>(
  integration: T,
): T {
  return integration;
}
