 ...
 * @example
 * ```
 * declare module '@khameleon/core/Pagination' {
...
export function Pagination({
  ...
  const label = labelFromProps ?? t('@khameleon.pagination.label');
  const previousLabel = t('@khameleon.pagination.previous');
  const nextLabel = t('@khameleon.pagination.next');
  const pageIndicatorsLabel = t('@khameleon.pagination.pageIndicators');
  const itemsPerPageLabel = t('@khameleon.pagination.itemsPerPage');
  ...
  const handlePageChange = (newPage: number) => {
    ...
    announce(
      computedTotalPages != null
        ? t('@khameleon.pagination.pageOfTotal', {
            current: newPage,
            total: computedTotalPages,
          })
        : t('@khameleon.pagination.pageAnnounce', {current: newPage}),
    );
          ...
          <>
            {pageRange.map((item, index) => {
              ...
              return (
                <Button
                  key={item}
                  label={t('@khameleon.pagination.goToPage', {page: item})}
                  aria-label={t('@khameleon.pagination.goToPage', {
                    page: item,
                  })}
                  variant="ghost"
                  size={buttonSize}
