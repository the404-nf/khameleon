...
export const BaseTypeahead = function BaseTypeahead<T extends SearchableItem>({
  ...
  const placeholder =
    placeholderFromProps ?? t('@khameleon.typeahead.searchPlaceholder');
  const emptySearchResultsText =
    emptySearchResultsTextFromProps ??
    t('@khameleon.typeahead.emptySearchResults');
  ...
  return (
    <>
      ...
      {isLoading && (
        <span
          role="status"
          aria-label={t('@khameleon.typeahead.loading')}
      ...
      {popover.render(
        <div
          id={listboxId}
          role="listbox"
          aria-label={t('@khameleon.typeahead.searchResults')}
          {...mergeProps(
