...
describe('AspectRatio', () => {
  ...
  describe('fit', () => {
    it('marks the child\'s direct parent with data-khameleon-aspect-ratio-override for fit="cover"', () => {
      render(
      ...
      expect(wrapper).toHaveAttribute(
        'data-khameleon-aspect-ratio-override',
        'cover',
      );
    });

    it('marks the child\'s direct parent with data-khameleon-aspect-ratio-override for fit="contain"', () => {
      render(
    ...
    it('emits no marker when fit is omitted (back-compat)', () => {
      ...
      const wrapper = screen.getByTestId('image').parentElement;
      expect(wrapper).not.toHaveAttribute('data-khameleon-aspect-ratio-override');
  ...
  describe('reset.css fit baseline rules', () => {
    ...
    // rules in reset.css keyed on the data-khameleon-aspect-ratio-override
    ...
    it('sizes the child to fill the box for cover and contain', () => {
      const fillMatch = resetCSS.match(
        /:where\(\[data-khameleon-aspect-ratio-override="cover"\], \[data-khameleon-aspect-ratio-override="contain"\]\)\s*>\s*:where\(\*\)\s*\{([^}]+)\}/,
      );
    ...
    it('crops media with object-fit: cover for fit="cover"', () => {
      const coverMatch = resetCSS.match(
        /:where\(\[data-khameleon-aspect-ratio-override="cover"\]\)\s*>\s*:where\(img, video\)\s*\{([^}]+)\}/,
      );
    ...
    it('letterboxes media with object-fit: contain for fit="contain"', () => {
      const containMatch = resetCSS.match(
        /:where\(\[data-khameleon-aspect-ratio-override="contain"\]\)\s*>\s*:where\(img, video\)\s*\{([^}]+)\}/,
      );
