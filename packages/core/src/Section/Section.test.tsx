...
describe('Section', () => {
  ...
  it('renders with variant="section" (default)', () => {
    ...
    expect(inner.className).toContain('khameleon-section');
  ...
  it('renders variant in khameleon class names', () => {
    const {container} = render(<Section variant="muted">Content</Section>);
    const inner = container.firstElementChild!.firstElementChild!;
    expect(inner.className).toContain('khameleon-section');
    expect(inner.className).toContain('muted');
  ...
  it('propagates explicit padding to nested sections via --khameleon-section-padding', () => {
    const {container} = render(
      <Section padding={6}>
        <Section data-testid="inner">Inner</Section>
      </Section>,
    );
    // Outer section's inner div should set --khameleon-section-padding
