...
export function MultiSelector<T extends MultiSelectorOptionType>({
  ...
  const placeholder =
    placeholderFromProps ?? t('@khameleon.multiSelector.selectPlaceholder');
  const selectAllLabel =
    selectAllLabelFromProps ?? t('@khameleon.multiSelector.selectAll');
  const searchPlaceholder =
    searchPlaceholderFromProps ?? t('@khameleon.multiSelector.searchPlaceholder');
    ...
    return (
      <div {...stylex.props(styles.searchWrapper)}>
        <input
          ...
          aria-label={t('@khameleon.multiSelector.searchOptions')}
          type="text"
          value={searchQuery}
      ...
      <div
        ...
        {hasClear && value.length > 0 && !isDisabled && (
          <button
            type="button"
            onClick={handleClear}
            aria-label={t('@khameleon.multiSelector.clearAll', {label})}
