...
export function PowerSearch({
  ...
  const label = labelFromProps ?? t('@khameleon.powersearch.label');
  const placeholder =
    placeholderFromProps ?? t('@khameleon.powersearch.placeholder');
  const popoverSaveButtonLabel =
    popoverSaveButtonLabelFromProps ?? t('@khameleon.powersearch.editor.apply');
  ...
  const resultCountText = useMemo((): string | null => {
    ...
    if (typeof resultCount === 'number') {
      return t('@khameleon.powersearch.resultCount', {count: resultCount});
    }
    return resultCount;
  }, [resultCount, t]);
