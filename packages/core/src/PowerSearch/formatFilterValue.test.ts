...
const t: TranslatorFn = (key, values) => {
  ...
  switch (key) {
    case '@khameleon.powersearch.valueEditor.itemsCount':
      return `${count} ${count === 1 ? 'item' : 'items'}`;
    case '@khameleon.powersearch.valueEditor.entitiesCount':
      return `${count} ${count === 1 ? 'entity' : 'entities'}`;
    case '@khameleon.powersearch.valueEditor.filtersCount':
      return `${count} ${count === 1 ? 'filter' : 'filters'}`;
    case '@khameleon.powersearch.valueEditor.dateRange':
      return 'date range';
    default:
      return key;
  }
