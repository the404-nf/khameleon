...
describe('Button', () => {
  ...
  it('renders the loading spinner with the inherit shade for every variant (#2717)', () => {
    ...
    for (const variant of [
      ...
      const spinner = container.querySelector('.khameleon-spinner');
  ...
  it('renders khameleon-* classes and data attributes for theme targeting', () => {
    render(<Button label="Test" variant="secondary" size="sm" />);
    const button = screen.getByRole('button');
    expect(button.className).toContain('khameleon-button');
