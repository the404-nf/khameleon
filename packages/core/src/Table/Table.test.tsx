...
describe('BaseTable', () => {
  ...
  describe('root element styling props (#3679)', () => {
    ...
    it('keeps the khameleon theme classes alongside a consumer className', () => {
      render(<Table data={users} columns={columns} className="custom-table" />);
      const table = screen.getByRole('table');
      expect(table.className).toContain('khameleon-base-table');
      expect(table.className).toContain('khameleon-table');
...
describe('Table', () => {
  ...
  it('wraps table in a scroll container', () => {
    ...
    expect(wrapper!.className).toContain('khameleon-table-scroll-wrapper');
  });
