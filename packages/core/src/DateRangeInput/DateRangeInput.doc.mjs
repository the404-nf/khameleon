...
export const docs = {
  ...
  props: [
    ...
    },
    {
      ...
      type: 'DateRange | null',
      description:
        'Selected date range or null. Import the `DateRange` type from `@khameleon/core/DateRangeInput`; it is `{start: ISODateString, end: ISODateString}`. Do NOT redeclare your own DateRange type; use the exported one so TypeScript structurally matches.',
      required: true,
    },
    {
      name: 'onChange',
    ...
    },
    {
      ...
      type: 'ISODateString',
      description:
        'Minimum selectable date. `ISODateString` is a template literal type (`\\`${number}${number}${number}${number}-${number}${number}-${number}${number}\\``). Pass a string literal like `"2026-01-28"`, not a runtime string variable. Import it from `@khameleon/core/Calendar` or use `as ISODateString` if computing the value dynamically.',
    },
    {
      name: 'max',
      type: 'ISODateString',
  ...
  theming: {
    targets: [
      {className: 'khameleon-date-range-input', visualProps: ['size', 'status']},
    ],
  },
  usage: {
    description:
...
export const docsDense = {
  ...
  propDescriptions: {
    ...
    value: 'selected range {start, end} or null; import DateRange type from @khameleon/core/DateRangeInput (do not redeclare)',
