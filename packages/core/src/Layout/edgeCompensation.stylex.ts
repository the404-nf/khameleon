 ...
 *
 * 1. **Components** mark themselves with `data-khameleon-edge-comp` (a passive
 *    data attribute — no styles attached).
 * 2. **Containers** use `:has(> [data-khameleon-edge-comp]:first-child)` and
 *    `:has(> [data-khameleon-edge-comp]:last-child)` to detect edge-adjacent
 ...
 */
export const EDGE_COMP_ATTR = 'data-khameleon-edge-comp';
