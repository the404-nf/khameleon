 ...
 * the `--khameleon-*` tokens that the component reads:
 *
 *   --khameleon-card-padding            (shorthand — all sides; emitted)
 *   --khameleon-card-padding-inline     (horizontal)
 *   --khameleon-card-padding-block-start
 *   --khameleon-card-padding-block-end
 *
 * Read order per level: `var(--khameleon-…, <next level>)`, terminating at
 * `--spacing-4`. Same pattern for section and dialog.
 *
 * ```ts
 * components: {
 *   card: { base: { padding: '20px' } },          // → --khameleon-card-padding: 20px
 ...
 * properties that themes can set. The pipeline emits the `--khameleon-*` names,
 * which the component reads via `var(--khameleon-…, …)`:
 *
 *   --khameleon-card-padding          (shorthand — all sides)
 *   --khameleon-card-padding-inline
 *   --khameleon-card-padding-inline-start
 *   --khameleon-card-padding-inline-end
 *   --khameleon-card-padding-block-start
 *   --khameleon-card-padding-block-end
 *
 ...
 * to override from @layer khameleon-theme. By reading from a higher-level token,
...
// Card padding chains: --khameleon-* then the next specificity level, terminating
...
const cardShorthand = `var(--khameleon-card-padding, ${SP4})`;
const cardInline = `var(--khameleon-card-padding-inline, ${cardShorthand})`;
const cardInlineStart = `var(--khameleon-card-padding-inline-start, ${cardInline})`;
const cardInlineEnd = `var(--khameleon-card-padding-inline-end, ${cardInline})`;
const cardBlockStart = `var(--khameleon-card-padding-block-start, ${cardShorthand})`;
const cardBlockEnd = `var(--khameleon-card-padding-block-end, ${cardShorthand})`;

// Section padding chains: --khameleon-* then the next specificity level, terminating
// at --spacing-4. Built as chained const strings (no function calls) so StyleX
// can statically analyze them; see naming.ts for the prefix policy.
const sectionShorthand = `var(--khameleon-section-padding, ${SP4})`;
const sectionInline = `var(--khameleon-section-padding-inline, ${sectionShorthand})`;
const sectionInlineStart = `var(--khameleon-section-padding-inline-start, ${sectionInline})`;
const sectionInlineEnd = `var(--khameleon-section-padding-inline-end, ${sectionInline})`;
const sectionBlockStart = `var(--khameleon-section-padding-block-start, ${sectionShorthand})`;
const sectionBlockEnd = `var(--khameleon-section-padding-block-end, ${sectionShorthand})`;

// Dialog padding chains: --khameleon-* then the next specificity level, terminating
// at --spacing-4. Built as chained const strings (no function calls) so StyleX
// can statically analyze them; see naming.ts for the prefix policy.
const dialogShorthand = `var(--khameleon-dialog-padding, ${SP4})`;
const dialogInline = `var(--khameleon-dialog-padding-inline, ${dialogShorthand})`;
const dialogInlineStart = `var(--khameleon-dialog-padding-inline-start, ${dialogInline})`;
const dialogInlineEnd = `var(--khameleon-dialog-padding-inline-end, ${dialogInline})`;
const dialogBlockStart = `var(--khameleon-dialog-padding-block-start, ${dialogShorthand})`;
const dialogBlockEnd = `var(--khameleon-dialog-padding-block-end, ${dialogShorthand})`;
 ...
 * @example
 * ```
 * import { container } from '@khameleon/core/Layout';
 * import * as stylex from '@stylexjs/stylex';
 *
