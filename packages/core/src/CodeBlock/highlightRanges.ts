...
function ensureDynamicHighlightType(tokenType: string): void {
  ...
  if (!dynamicStyleSheet) {
    ...
    style.setAttribute('data-khameleon-highlight-dynamic', '');
  ...
  const name = `khameleon-${tokenType}`;
  const colorVar = `var(--color-syntax-${tokenType}, currentColor)`;
  dynamicStyleSheet.insertRule(
    `.khameleon-codeblock code::highlight(${name}), .khameleon-codeeditor code::highlight(${name}) { color: ${colorVar}; }`,
  );
}
