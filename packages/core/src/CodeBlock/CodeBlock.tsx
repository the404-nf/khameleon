...
export interface CodeBlockProps extends BaseProps<HTMLPreElement> {
   ...
   * `@khameleon/core/theme/syntax` or a theme created with
  ...
  for (const token of tokens) {
    ...
    parts.push(
      <span
        ...
        className={`khameleon-token-${token.type} xds-token-${token.type}`}>
...
export function CodeBlock({
  ...
  const copyButtonEl = hasCopyButton ? (
    <button
      ...
      aria-label={copied ? t('@khameleon.codeBlock.copied') : t('@khameleon.codeBlock.copyCode')}
      {...stylex.props(
        styles.copyButton,
  ...
  const codeBody = (
    <div
      ...
      tabIndex={0}
      role="group"
      aria-label={languageLabel ?? t('@khameleon.codeBlock.code')}
