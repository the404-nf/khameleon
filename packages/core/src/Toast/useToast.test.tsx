...
function newestMediaAttr(scope: ParentNode): string | null {
  const nodes = scope.querySelectorAll('[data-khameleon-media]');
  const last = nodes[nodes.length - 1];
  return last ? last.getAttribute('data-khameleon-media') : null;
}
...
async function dismissAllFallbackToasts(): Promise<void> {
  const fallback = document.querySelector('[data-khameleon-toast-fallback]');
...
describe('useToast fallback viewport theme mode', () => {
  ...
  it('resolves the app mode (light) instead of OS preference (dark) with no LayerProvider', async () => {
    ...
    await waitFor(() => {
      expect(
        document.querySelector(
          '[data-khameleon-toast-fallback] [data-khameleon-media]',
        ),
      ).not.toBeNull();
    });
    ...
    expect(
      newestMediaAttr(document.querySelector('[data-khameleon-toast-fallback]')!),
    ).toBe('dark');
  });
  ...
  it('mirrors <html data-theme> and data-khameleon-theme onto the fallback container', async () => {
    mockMatchMedia(false);
    ...
    await waitFor(() => {
      const fallback = document.querySelector(
        '[data-khameleon-toast-fallback]',
      ) as HTMLElement;
      expect(fallback.getAttribute('data-theme')).toBe('dark');
      expect(fallback.getAttribute('data-khameleon-theme')).toBe('test');
