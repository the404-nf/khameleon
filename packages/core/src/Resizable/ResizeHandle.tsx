...
const styles = stylex.create({
  ...
  // Pill base — themes target .khameleon-resize-handle-pill for size/shape.
  pill: {
    position: 'absolute',
    zIndex: 2,
    pointerEvents: 'none',
 ...
 *
 * The pill element uses class `khameleon-resize-handle-pill` for theme targeting.
 *
 * @example
 * ```
 * <ResizeHandle
...
export function ResizeHandle({
  ...
  const label = labelFromProps ?? t('@khameleon.resizable.handle.label');
  ...
  return (
    <div
      ...
      />
      {/* Pill grip indicator — themed via .khameleon-resize-handle-pill */}
      {children ?? (
        <div
          {...mergeProps(
