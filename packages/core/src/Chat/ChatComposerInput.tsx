...
function serialize(node: Node): string {
  ...
  for (const child of Array.from(node.childNodes)) {
    if (child.nodeType === Node.TEXT_NODE) {
      ...
      if (child.hasAttribute('data-khameleon-token')) {
        result += child.getAttribute('data-khameleon-token-value') ?? '';
...
export function ChatComposerInput(props: ChatComposerInputProps) {
  ...
  const label = labelFromProps ?? t('@khameleon.chat.composerInput.label');
  const placeholder =
    placeholderFromProps ??
    composerCtx?.placeholder ??
    t('@khameleon.chat.composer.placeholder');
  ...
  const emitChange = useCallback(() => {
    ...
    const hasTokens =
      editableRef.current.querySelector(
        '[data-khameleon-token], [data-khameleon-dictation-interim]',
      ) != null;
      ...
      if (e.key === 'Backspace') {
        ...
        if (selection && selection.isCollapsed && selection.rangeCount > 0) {
          ...
          if (
            ...
            startContainer.previousSibling.hasAttribute('data-khameleon-token')
          ) {
...
export function ChatComposerTokenElement({token}: {token: ChatComposerToken}) {
  return (
    <span
      data-khameleon-token=""
      data-khameleon-token-value={token.value}
