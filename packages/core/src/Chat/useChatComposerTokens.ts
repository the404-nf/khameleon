 ...
 * - Serialization awareness (data-khameleon-token attributes)
 *
...
function isInsideToken(node: Node): HTMLElement | null {
  ...
  while (current) {
    if (
      ...
      current.hasAttribute('data-khameleon-token')
    ) {
      return current;
    }
...
export function useChatComposerTokens({
  ...
  const insertToken = useCallback(
    (token: ChatComposerToken): string | undefined => {
      ...
      span.setAttribute('data-khameleon-token', '');
      span.setAttribute('data-khameleon-token-value', token.value);
      span.setAttribute('data-khameleon-token-id', id);
  ...
  const expandToken = useCallback(
    (id: string) => {
      ...
      const {span} = portal;
      const value = span.getAttribute('data-khameleon-token-value') ?? '';
