  ...
  sections: [
    {
      ...
      content: [
        ...
        {
          type: 'prose',
          text: 'Use CSS variables for ordinary DOM styling because they inherit through the tree, follow `data-theme` color mode, respect nested `data-khameleon-theme` scopes, and update when themes switch. Use token resolver APIs only for non-CSS consumers such as SVG attribute values, canvas, chart configuration, color calculations, or static config generation.',
        },
        {
          type: 'prose',
          text: 'For available token names and values, run `npx khameleon docs tokens`. Focused references are also available with `npx khameleon docs color`, `npx khameleon docs spacing`, `npx khameleon docs shape`, `npx khameleon docs typography`, `npx khameleon docs elevation`, and `npx khameleon docs motion`.',
        },
      ],
    },
    {
      ...
      content: [
        {
          ...
          items: [
            ...
            'Let the system own color mode. The root Theme syncs `data-theme="light|dark"` and `data-khameleon-theme` to `<html>` for portals and first-level theme scope.',
  ...
  padding: var(--spacing-4);
    ...
    },
    {
      ...
      content: [
        {
          type: 'prose',
          text: 'For StyleX styles, prefer the typed token exports from `@khameleon/core/theme/tokens.stylex`. They provide autocomplete and catch token-name typos while still resolving through the same system CSS variables at runtime.',
        },
        {
          type: 'code',
          lang: 'tsx',
...
import {colorVars, spacingVars, radiusVars} from '@khameleon/core/theme/tokens.stylex';
  ...
  panel: {
    ...
    },
    {
      ...
      content: [
        {
          type: 'prose',
          text: 'The Tailwind v4 bridge at `@khameleon/core/tailwind-theme.css` maps Tailwind theme variables to system CSS variables with `@theme inline`, so utility classes like `text-primary`, `bg-surface`, `border-border`, `rounded-lg`, and `shadow-md` stay in sync with the active theme.',
        },
        {
          type: 'code',
          lang: 'css',
      ...
      content: [
        {
          ...
          style: 'ordered',
          items: [
            'Import the reset/base CSS and a theme CSS file early enough for first paint. For production SSR, prefer built themes from `npx khameleon theme build` or published `/built` theme imports plus `theme.css`.',
            ...
            'Use `npx khameleon docs tokens` and focused token docs when building mappings. Keep mappings small at first: text, surface/body/card/popover, border, accent, status, spacing, radius, typography, shadow.',
