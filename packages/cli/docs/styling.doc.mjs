...
export const docs = {
  ...
  category: 'guide',
      content: [
        ...
        {
          type: 'prose',
          text: 'All approaches resolve to the same design tokens, so theming and dark mode work regardless of which you choose. For external styling libraries, run `npx khameleon docs styling-libraries`; it covers Tailwind, StyleX, Panda, Chakra, MUI, CSS-in-JS, CSS Modules, Sass, and `useTheme()` for non-CSS processing.',
        },
      ],
    },
    {
  ...
  category: 'guide',
      content: [
        ...
        },
        {
          ...
          code: `@layer reset, theme, base, khameleon-base, khameleon-theme, components, utilities;
...
<Dialog isOpen={isOpen} onClose={close} xstyle={overrides.dialog}>
  ...
  category: 'guide',
      content: [
        {
          type: 'prose',
          text: 'When external CSS needs to target an Khameleon component by prop or state, combine the stable component class with reflected data attributes. The component class identifies the component (`.khameleon-button`, `.khameleon-card`); data attributes identify the axis and value (`data-variant`, `data-size`, `data-level`, etc.). This is the preferred selector surface for new CSS because it is explicit and collision-resistant.',
        },
        {
          type: 'code',
          lang: 'css',
          code: `.my-app .khameleon-button[data-variant="primary"] {
  /* primary buttons in this app context */
}

.my-app .khameleon-button[data-variant="primary"][data-size="sm"] {
  /* small primary buttons */
}

.my-app .khameleon-heading[data-level="2"] {
...
// preferred selector attrs: data-level="2"`,
        ...
        {
          type: 'prose',
          text: 'For systematic theming, use defineTheme component overrides instead of raw CSS selectors. defineTheme keeps the higher-level `prop:value` API (`variant:primary`, `size:sm`) and handles selector generation for you. Run `npx khameleon docs theme` for the full theming guide.',
        },
      ],
    },
    {
...
<Card xstyle={styles.surface} />`,
        },
        {
          ...
          code: `import {colorVars, spacingVars, radiusVars} from '@khameleon/core/theme/tokens.stylex';
...
const styles = stylex.create({
  highlight: {
    ...
    borderRadius: radiusVars['--radius-element'],
        ...
        {
          type: 'prose',
          text: 'See `npx khameleon docs tokens` for the full token reference (all spacing, color, radius, shadow, and typography tokens with values). See `npx khameleon docs theme` for how to override tokens via defineTheme.',
        },
      ],
    },
    {
...
export default stylexPlugin({
  rsOptions: {
    // Resolve @khameleon/core's StyleX so swizzled component source compiles.
    aliases: {'@/*': ['./src/*']},
  ...
  // your existing Next.js config
        ...
        },
        {
          ...
          items: [
            ...
            'Pure theming (defineTheme + khameleon theme build) needs NO StyleX compiler; only swizzled/authored StyleX source does.',
          ],
        },
      ],
    },
