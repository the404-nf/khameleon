 ...
 *     KhameleonIntegrationIssue encountered is collected into a private set and
...
import {KhameleonConfigSchema} from './config-schema.mjs';
...
/**
 * Find khameleon.config.{ts,mjs,js} as a sibling of the nearest package.json.
...
export function findConfigPath(startDir = process.cwd()) {
  ...
  if (present.length > 1) {
    throw new Error(
      `Multiple Khameleon config files found in ${root} (${present
...
export class Project {
  ...
  /** @type {string|null} */
  #configPath;
  /** @type {import('../types/config').KhameleonConfig} */
  #config;
  /** @type {string[]} */
  #integrations;
  /** @type {Array<object>} */
  ...
  /** @type {string} */
  #hash;
  /** @type {import('../types/integration').KhameleonIntegrationIssue[]} */
  #issues = [];
  /**
   ...
   * @param {object} init.config validated KhameleonConfig surface
  ...
  static async load(cwd = process.cwd(), {cache} = {}) {
    ...
    if (configPath) {
      config = await loadModuleWithSchema(configPath, KhameleonConfigSchema, {
        label: 'khameleon.config',
      });
   ...
   * @param {string} pkg
   * @param {import('../types/integration').KhameleonIntegrationIssue} issue
   */
  #pushIssue(pkg, issue) {
   ...
   *
   * @returns {Promise<import('../types/integration').KhameleonIntegrationIssue[]>}
   */
  async issues() {
    for (const integration of this.#loadedIntegrations) {
      await this.#collectIssues(integration);
