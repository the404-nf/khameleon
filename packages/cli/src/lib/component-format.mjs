 ...
 *
 * Override keys are the component's stable class name with the `khameleon-`
 * namespace prefix stripped — `generateThemeRules` re-adds the prefix when it
 * builds the `.khameleon-*` selector. So `khameleon-base-table` → key `base-table`
 * (→ `.khameleon-base-table`), and `khameleon-button` → key `button`.
 *
 * Keep the `khameleon-` literal in sync with packages/core/src/naming.ts
...
function targetKey(target) {
  return target.className.replace(/^khameleon-/, '');
}
...
function formatSubComponent(comp) {
  ...
  if (table) {
    ...
    out.push(`See \`khameleon component ${comp.name}\` for props and usage.\n`);
  }
  return out;
}
 ...
 * @param {string|null} [options.importHint] - Import path hint (e.g. '@khameleon/core/Button')
 */
...
export function formatCompact(docs, componentName, importHint) {
  ...
  // without the Khameleon prefix. componentName is already bare post-P4.
