 ...
 * try/catch in `bin/khameleon.mjs` never sees them). The result: a --json
 ...
 *      envelope path (so `khameleon bogus-cmd --json` gets exit 1 + envelope
 ...
 * subcommand (e.g. `khameleon component --help --json`).
 *
 * @param {import('commander').Command} cmd
 * @returns {{apiVersion: number, type: 'help', data: object}}
 */
export function buildHelpEnvelope(cmd) {
  // Reconstruct the fully-qualified command name (e.g. "khameleon theme build").
  const nameParts = [];
  let c = cmd;
  while (c) {
...
/**
 * Map a Commander parse-error code (and its message) to a stable Khameleon error
 ...
 *   commander.unknownCommand      — `khameleon bogus-cmd`
 *   commander.unknownOption       — `--bogus-flag`
 *   commander.missingArgument     — `khameleon theme build` (no <file>)
