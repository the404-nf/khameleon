...
/**
 * Scaffold a consumer project (package.json + khameleon.config.mjs) under a
...
beforeEach(() => {
  ...
  tmpDir = fs.mkdtempSync(path.join(process.cwd(), '.khameleon-project-test-'));
...
describe('findConfigPath', () => {
  it('rejects multiple config files at the same root', () => {
    ...
    fs.writeFileSync(
      path.join(tmpDir, 'khameleon.config.js'),
      `module.exports = {};\n`,
    );
    expect(() => findConfigPath(tmpDir)).toThrow(/Multiple Khameleon config files/);
  });
});
...
describe('Project.issuesUrl routing', () => {
  it('routes core to config.issuesUrl, falling back to the default', async () => {
    ...
    expect(project.issuesUrl({package: '@khameleon/core'})).toBe(
