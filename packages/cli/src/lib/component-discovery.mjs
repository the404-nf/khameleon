...
/**
 * @file Component discovery — find, list, and resolve Khameleon components
 */
...
export const CORE_PACKAGE = '@khameleon/core';
...
/**
 * Auto-discover components by scanning for Khameleon*.tsx files in core/src/.
 *
...
export function discoverComponents(coreDir) {
  ...
  for (const entry of topEntries) {
    ...
    // Check both {Name}.doc.mjs and Khameleon{Name}.doc.mjs naming conventions
...
export function findComponentReadme(coreDir, name) {
  ...
  // Direct match: src/{name}/{Name}.doc.mjs or src/{name}/Khameleon{Name}.doc.mjs
  ...
  // Nested match: src/*/{name}/{Name}.doc.mjs or src/*/{name}/Khameleon{Name}.doc.mjs
  ...
  // Per-component doc in a parent directory: src/*/{Name}.doc.mjs or src/*/Khameleon{Name}.doc.mjs
  for (const entry of entries) {
    if (!entry.isDirectory()) continue;
...
/**
 * Find the main source file for a component (Khameleon*.tsx, excluding tests).
 ...
 * bare `@khameleon/core` root instead of its tree-shakeable subpath.
 */
export function findComponentSource(coreDir, name) {
  const srcDir = path.join(coreDir, 'src');
  // Try the prefixed forms (`XDSButton.tsx`) first since that is the current
  // on-disk convention, then the bare forms (`Button.tsx`) that the Khameleon-prefix
...
export function resolveImportPath(coreDir, componentName) {
  ...
  if (exactMatch) {
    return `@khameleon/core/${exactMatch.slice(2)}`;
  }

  const sourcePath = findComponentSource(coreDir, componentName);
  if (!sourcePath) return '@khameleon/core';
  ...
  if (topMatch) {
    return `@khameleon/core/${topMatch.slice(2)}`;
  }

  return '@khameleon/core';
}
 ...
 *
 * Core records carry `package: '@khameleon/core'`, `issuesUrl: undefined`
