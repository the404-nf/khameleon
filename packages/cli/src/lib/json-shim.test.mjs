...
const CLI_BIN = path.resolve(__dirname, '../../bin/khameleon.mjs');
...
describe('--json shim: --help renders JSON envelope', () => {
  it('khameleon --help --json emits a help envelope (not raw text), exit 0', () => {
    ...
    expect(parsed.data.command).toBe('khameleon');
  ...
  it('khameleon component --help --json emits a subcommand help envelope', () => {
    const {status, stdout} = runCli(['component', '--help', '--json']);
    expect(status).toBe(0);
    const parsed = parseJson(stdout);
    expect(parsed.apiVersion).toBe(1);
    expect(parsed.type).toBe('help');
    expect(parsed.data.command).toBe('khameleon component');
    // Should expose its options
    const flags = parsed.data.options.map((o) => o.flags);
    expect(flags).toContain('--list');
  });

  it('khameleon theme build --help --json emits a nested subcommand help envelope', () => {
    const {status, stdout} = runCli(['theme', 'build', '--help', '--json']);
    expect(status).toBe(0);
    const parsed = parseJson(stdout);
    expect(parsed.apiVersion).toBe(1);
    expect(parsed.type).toBe('help');
    expect(parsed.data.command).toBe('khameleon theme build');
    expect(parsed.data.usage).toMatch(/<file>/);
  });
});

describe('--json shim: parse errors emit JSON error envelope', () => {
  it('khameleon theme build --json (missing required arg) emits error envelope, exit 1', () => {
  ...
  it('khameleon --bogus-flag --json (unknown option) emits error envelope, exit 1', () => {
  ...
  it('khameleon component --json --bogus-flag (unknown option on subcmd) emits error envelope, exit 1', () => {
...
describe('--json shim: unknown subcommand emits error envelope', () => {
  it('khameleon bogus-cmd --json emits error envelope, exit 1 (not exit 0 + help)', () => {
...
describe('--json shim: invalid --detail choice', () => {
  it('khameleon --detail bogus --json emits a single error envelope, exit 1', () => {
...
describe('--json shim: invalid --lang choice', () => {
  it('khameleon docs color --lang fr --json emits a single error envelope, exit 1', () => {
  ...
  it('khameleon docs color --lang fr (no --json) writes to stderr and exits 1', () => {
  ...
  it('khameleon docs color --lang zh is accepted (exit 0)', () => {
    const {status} = runCli(['docs', 'color', '--lang', 'zh']);
    expect(status).toBe(0);
  });

  it('khameleon docs color --lang en is accepted (exit 0)', () => {
...
describe('--json shim: non-JSON behavior is preserved', () => {
  it('khameleon theme build (no --json, missing arg) writes to stderr and exits 1', () => {
  ...
  it('khameleon --bogus-flag (no --json) writes to stderr and exits 1', () => {
  ...
  it('khameleon bogus-cmd (no --json) writes to stderr and exits 1', () => {
  ...
  it('khameleon --detail bogus (no --json) writes to stderr and exits 1', () => {
  ...
  it('khameleon --help (no --json) writes raw help text to stdout, exit 0', () => {
    const {status, stdout, stderr} = runCli(['--help']);
    expect(status).toBe(0);
    expect(stdout).toMatch(/^Usage: khameleon/);
    expect(stderr).toBe('');
  });
});
