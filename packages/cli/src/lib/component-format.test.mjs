...
describe('formatFull sub-component rendering', () => {
  ...
  it('does not print "undefined" for a bare {name} sub-component', () => {
    ...
    expect(out).toContain('khameleon component XDSRadioListItem');
  });
  ...
  it('renders a full props table for a sub-component that has inline props', () => {
    ...
    expect(out).not.toContain('khameleon component XDSButtonGroup');
  });
});

describe('formatFull theming override keys', () => {
  // Regression guard: the `defineTheme` component-override key is the stable
  // class name with the `khameleon-` prefix stripped — `generateThemeRules`
  // re-adds the prefix to build the `.khameleon-*` selector. targetKey() used to
  // strip a dead `xds-` prefix (from before the khameleon rename), so the printed
  // example advertised `khameleon-*` keys. Those double-prefix to
  // `.khameleon-khameleon-*` at runtime and silently match nothing. See issue #3458.
  it('prints override keys without the khameleon- prefix (single-word component)', () => {
    const docs = {
      name: 'Button',
      description: 'A button.',
      theming: {targets: [{className: 'khameleon-button', visualProps: ['variant']}]},
    };
    const out = formatFull(docs);

    expect(out).toContain("'button': {");
    // The full DOM class must not appear as an override key.
    expect(out).not.toContain("'khameleon-button': {");
  });

  it('prints override keys without the khameleon- prefix (compound Table classes)', () => {
    const docs = {
      name: 'Table',
      description: 'A table.',
      theming: {
        targets: [
          {className: 'khameleon-base-table'},
          {className: 'khameleon-table-cell'},
        ],
      },
    };
    const out = formatFull(docs);

    // Correct keys: class name minus the khameleon- prefix.
    ...
    expect(out).not.toContain("'khameleon-base-table': {");
    expect(out).not.toContain("'khameleon-table-cell': {");
  });
});
