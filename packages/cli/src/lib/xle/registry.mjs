...
/**
 * @file XLE component registry — built from @khameleon/core .doc.mjs metadata.
 *
...
export async function buildRegistry({cwd = process.cwd()} = {}) {
  ...
  if (!coreDir) {
    throw new Error('Could not find @khameleon/core package — run from an XDS workspace');
  }
  ...
  const upgradeFromDoc = (rawName, props, fallbackDir) => {
    ...
    const name = normalizeName(rawName);
    // Prefer the component's own export subpath (Heading → @khameleon/core/Heading);
