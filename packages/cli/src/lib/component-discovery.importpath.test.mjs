...
describe('resolveImportPath reproduces authored hook importPaths', () => {
  ...
  // the `.ts` fix these fell back to bare `@khameleon/core`; the derived
  // subpath must now match the value each hook's doc currently authors.
  const cases = [
    ['useMediaQuery', '@khameleon/core/hooks'],
    ['useResizable', '@khameleon/core/Resizable'],
    ['useTheme', '@khameleon/core/theme'],
    ['useFocusTrap', '@khameleon/core/hooks'],
    ['useOverflow', '@khameleon/core/hooks'],
  ];
  ...
  it('never falls back to the bare package root for a .ts hook', () => {
    for (const [name] of cases) {
      expect(resolveImportPath(coreDir, name)).not.toBe('@khameleon/core');
    }
  });
});
