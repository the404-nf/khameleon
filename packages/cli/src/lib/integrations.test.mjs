...
function writeManifestPackage(dir, {basename = 'khameleon.integration.mjs', body}) {
...
beforeEach(() => {
  ...
  tmpDir = fs.mkdtempSync(
    path.join(process.cwd(), '.khameleon-integration-test-'),
  );
  fs.writeFileSync(
...
describe('configured integrations', () => {
  ...
  it('errors when the package has multiple root manifests', async () => {
    ...
    fs.writeFileSync(
      path.join(pkgDir, 'khameleon.integration.js'),
      `module.exports = {};\n`,
    );
    await expect(loadIntegrations(['@acme/widgets'], {cwd: tmpDir})).rejects.toThrow(
