...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'khameleon-docs-test-'));
...
describe('registerDocs', () => {
  it('lists available topics when no topic given', async () => {
    ...
    await program.parseAsync(['node', 'khameleon', 'docs']);
  ...
  it('errors for unknown topic', async () => {
    ...
    await expect(
      program.parseAsync(['node', 'khameleon', 'docs', 'nonexistent']),
...
describe('hyphenated doc filenames', () => {
  ...
  it('loads a hyphenated topic by name', async () => {
    const program = createProgram();
    await program.parseAsync(['node', 'khameleon', 'docs', 'getting-started']);
...
describe('migration docs', () => {
  ...
  it('loads migration docs by topic name', async () => {
    const program = createProgram();
    await program.parseAsync(['node', 'khameleon', 'docs', 'migration']);
