 ...
 * `khameleon theme build` (#3391 companion: #3371).
 *
 ...
 *
 * Building `khameleon theme build` requires a compiled @khameleon/core, so this
...
let tmpDir;
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'khameleon-build-theme-variants-'));
});
afterEach(() => {
...
describe('theme build custom-variant augmentations', () => {
  it('targets the real (un-prefixed) core interface for a custom variant', () => {
    ...
    expect(dts).toContain("declare module '@khameleon/core/Button'");
  ...
  it('skips props with no augmentation point (Button size, Heading type)', () => {
    ...
    expect(dts).not.toContain("declare module '@khameleon/core/Heading'");
  });
