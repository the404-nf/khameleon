...
/**
 * @file Path-traversal regression tests for `khameleon swizzle --output`.
 *
...
// node_modules/@khameleon/core/src/Button so findCoreDir succeeds, and
...
function buildFakeRepo(tmpDir) {
  ...
  fs.writeFileSync(path.join(core, 'package.json'), '{"name":"@khameleon/core"}');
...
let tmpDir;
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'khameleon-swizzle-paths-'));
});
afterEach(() => {
...
describe('swizzle path safety', () => {
  ...
  it('requires --overwrite in non-interactive mode when files already exist', () => {
    const {project} = buildFakeRepo(tmpDir);
    const outDir = path.join(project, 'components', 'khameleon', 'Button');
