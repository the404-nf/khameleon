...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'khameleon-json-contract-'));
});
...
describe('--json contract: rejects before side effects', () => {
  it('khameleon init --json --features agents does not write .claude/CLAUDE.md', () => {
  ...
  it('khameleon init --json --all does not write any files', () => {
  ...
  it('khameleon theme --json (parent, no subcommand) rejects without printing help', () => {
  ...
  it('khameleon postinstall --json rejects (hidden command not on allowlist)', () => {
...
describe('--json contract: supported commands emit valid envelopes', () => {
  it('khameleon --version --json emits { type: "version", data: { version } }', () => {
  ...
  it('khameleon --json (no subcommand) emits a help envelope', () => {
  ...
  it('khameleon upgrade --json --list returns the codemod list', () => {
  ...
  it('khameleon upgrade --json (already up to date) emits upgrade.status', () => {
    // Force a no-op range: from > installed target.
    const coreDir = path.join(tmpDir, 'node_modules', '@khameleon', 'core');
    fs.mkdirSync(coreDir, {recursive: true});
    fs.writeFileSync(
      path.join(coreDir, 'package.json'),
      JSON.stringify({name: '@khameleon/core', version: '0.0.1'}, null, 2),
    );
  ...
  it('khameleon discover --json (no config) includes meta.configured=false', () => {
  ...
  it('khameleon template --json emits a typed envelope', () => {
  ...
  it('khameleon docs --json emits a typed envelope', () => {
  ...
  it('khameleon doctor --json emits a doctor envelope with checks + summary', () => {
    // Run in a bare tmp dir → @khameleon/core won't resolve → a FAIL → exit 1.
