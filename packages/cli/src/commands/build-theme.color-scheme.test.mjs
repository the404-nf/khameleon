...
/**
 * @file Regression test for the `color-scheme` declaration `khameleon theme build`
 * emits alongside `light-dark()` token output (see #3658).
 *
 * `khameleon theme build` injects a `color-scheme` declaration into
 * `@layer khameleon-theme` whenever the generated CSS contains `light-dark()`,
 ...
 * `@layer khameleon-theme` being declared after `@layer reset` in layer order):
 *
 ...
 *
 * Building `khameleon theme build` requires a compiled @khameleon/core (there is no in-CLI
...
// `khameleon theme build` imports the compiled @khameleon/core/theme entry. Build core
...
let tmpDir;
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'khameleon-build-theme-color-scheme-'));
});
afterEach(() => {
...
describe('theme build color-scheme output', () => {
  it('emits the mode-aware color-scheme rules in @layer khameleon-theme for a light-dark() theme', () => {
    ...
    // ...and specifically inside @layer khameleon-theme, not @layer reset (the
    // declaration must sit alongside the light-dark() token/component output
    // it resolves, not the zero-specificity prose defaults).
    const themeLayerStart = css.indexOf('@layer khameleon-theme');
