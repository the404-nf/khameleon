...
/**
 * @file Integration-routing tests for `khameleon swizzle`.
 *
 ...
 * fixtures: a fake @khameleon/core under node_modules plus, for the
 * integration cases, a configured integration package (khameleon.config.mjs +
 * khameleon.integration.mjs + a `components` dir) — all under node_modules so the
 ...
 * escaping import, plus a colocated test file. Writes khameleon.config.mjs at the
...
function buildIntegration(project, {issuesUrl, componentName = 'MetaAppShell'} = {}) {
  ...
  fs.writeFileSync(
    path.join(intDir, 'khameleon.integration.mjs'),
  ...
  fs.writeFileSync(
    path.join(project, 'khameleon.config.mjs'),
...
let project;
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'khameleon-swizzle-routing-'));
...
describe('swizzle — core feedback routing via config', () => {
  it('routes core feedback to config.issuesUrl when set', () => {
    ...
    expect(env.data.package).toBe('@khameleon/core');
    ...
    const out = fs.readFileSync(
      path.join(project, 'components', 'khameleon', 'Button', 'Button.tsx'),
      'utf-8',
    );
    expect(out).toContain(`from '@khameleon/core/theme'`);
  ...
  it('falls back to the default issues URL when config has none', () => {
    ...
    expect(env.data.feedback.issuesUrl).toBe(
      'https://github.com/the404-nf/khameleon/issues/new',
    );
  });
});
...
describe('swizzle — integration-owned components', () => {
  it('copies the component dir (excluding test/doc), rewrites escaping imports to the owner package, routes feedback to the integration issuesUrl', () => {
    ...
    const outDir = path.join(project, 'components', 'khameleon', 'MetaAppShell');
...
describe('swizzle — ambiguous ownership', () => {
  it('errors when a name is owned by core + an integration and no --package is given', () => {
    ...
    expect(pkgs).toContain('@khameleon/core');
  ...
  it('--package resolves an ambiguous name to core', () => {
    ...
    const result = runCli(
      ['--json', 'swizzle', 'Button', '--package', '@khameleon/core', '-f'],
      project,
    );
    expect(result.code).toBe(0);
    const env = JSON.parse(result.stdout);
...
/**
 * Build a fake @khameleon/core with a component that imports StyleX directly
...
describe('swizzle — StyleX build setup note (#3373)', () => {
  it('reports usesStyleX and prints a setup note for StyleX components', () => {
    ...
    expect(humanResult.stdout).toMatch(/khameleon docs styling/);
  });
