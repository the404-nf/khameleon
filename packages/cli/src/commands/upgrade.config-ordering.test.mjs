 ...
 * (package.json + khameleon.config.mjs) under a REPO-LOCAL temp dir (so Vite
...
beforeEach(() => {
  ...
  // Repo-local temp dir: under cwd so Vite allows the khameleon.config import.
  tmpDir = fs.mkdtempSync(path.join(process.cwd(), '.khameleon-upgrade-order-'));
...
function writeInstalledCore(version) {
  const dir = path.join(tmpDir, 'node_modules', '@khameleon', 'core');
  fs.mkdirSync(dir, {recursive: true});
  fs.writeFileSync(
    path.join(dir, 'package.json'),
    JSON.stringify({name: '@khameleon/core', version}),
  );
}

function writeConfig(body) {
  fs.writeFileSync(path.join(tmpDir, 'khameleon.config.mjs'), body);
}
...
describe('upgrade — core codemods run before config load', () => {
  it('dry-run: legacy layout.components is reported as fixable, not an abort', async () => {
    ...
    expect(result.data.suggestedCommand).toBe(
      'khameleon upgrade --from 0.1.2 --codemod migrate-layout-components-to-experimental --apply',
    );
    ...
    const onDisk = fs.readFileSync(
      path.join(tmpDir, 'khameleon.config.mjs'),
      'utf-8',
    );
