...
/**
 * @file Shared test helper: build @khameleon/core once, race-free.
 *
 * `khameleon theme build` imports the compiled @khameleon/core/theme entry
 ...
 * suites each ran `if (!exists) pnpm -F @khameleon/core build` in their own
...
const LOCK_DIR = path.join(os.tmpdir(), 'khameleon-core-build.lock');
...
function buildCore() {
  execFileSync('pnpm', ['-F', '@khameleon/core', 'build'], {
    cwd: REPO_ROOT,
    stdio: 'pipe',
