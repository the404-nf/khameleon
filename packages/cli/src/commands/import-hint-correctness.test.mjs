...
describe('import hint correctness', () => {
  ...
  describe('every component resolves to a tree-shakeable subpath (not bare @khameleon/core)', () => {
    for (const name of individualComponents) {
      it(`${name} resolves to @khameleon/core/<subpath>, not bare @khameleon/core`, () => {
        const importPath = resolveImportPath(coreDir, name);

        // Must never be empty
        expect(importPath).toBeTruthy();

        // Must resolve to a specific subpath — bare @khameleon/core is not tree-shakeable
        expect(importPath).not.toBe('@khameleon/core');

        // The subpath must exist in package.json exports
        const subpath = importPath.replace('@khameleon/core/', '');
  ...
  describe('import hint case matches package.json exports exactly', () => {
    for (const name of individualComponents) {
      it(`${name} import path has correct casing`, () => {
        const importPath = resolveImportPath(coreDir, name);
        if (importPath === '@khameleon/core') return; // skip bare fallback
  ...
  describe('CLI --detail brief shows correct import path', () => {
    ...
    const representative = [
      {name: 'Button', expected: '@khameleon/core/Button'},
      {name: 'Theme', expected: '@khameleon/core/theme'},
      {name: 'CheckboxInput', expected: '@khameleon/core/CheckboxInput'},
      {name: 'Table', expected: '@khameleon/core/Table'},
      {name: 'TextInput', expected: '@khameleon/core/TextInput'},
      {name: 'Layout', expected: '@khameleon/core/Layout'},
      {name: 'AppShell', expected: '@khameleon/core/AppShell'},
      {name: 'Card', expected: '@khameleon/core/Card'},
      {name: 'Dialog', expected: '@khameleon/core/Dialog'},
      {name: 'TabList', expected: '@khameleon/core/TabList'},
    ];

    for (const {name, expected} of representative) {
      it(`npx khameleon component ${name} --detail brief shows ${expected}`, () => {
  ...
  describe('CLI --detail full shows import hint', () => {
    ...
    for (const name of representative) {
      it(`npx khameleon component ${name} (full) includes import statement`, () => {
        const result = runCli(['component', name]);
        expect(result.code).toBe(0);
        // The PR adds: **Import:** `import {XDS...} from '...';`
        expect(result.stdout).toMatch(/import\s*\{/);
        expect(result.stdout).toContain('@khameleon/core');
      });
    }
  });
  ...
  describe('group names do NOT resolve to incorrect import paths', () => {
    ...
    for (const group of groups) {
      it(`group "${group}" does not produce a subpath import`, () => {
        // Groups should either fail to find a source file (returning bare @khameleon/core)
        // or correctly return the group-level export if one exists
        const importPath = resolveImportPath(coreDir, group);
        if (importPath !== '@khameleon/core') {
  ...
  describe('import path matches actual source file location', () => {
    for (const name of individualComponents) {
      it(`${name} import path leads to its source directory`, () => {
        ...
        const importPath = resolveImportPath(coreDir, name);
        if (importPath === '@khameleon/core') return;
