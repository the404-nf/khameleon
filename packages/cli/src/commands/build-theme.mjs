 ...
 *
 * Usage:
 *   npx khameleon theme build ./src/themes/ocean.ts
 *   npx khameleon theme build ./src/themes/ocean.ts --out ./dist/ocean.css
 */
...
// Import shared theme processing from core. `khameleon theme build` MUST produce the
...
// action). A built, resolvable `@khameleon/core` is a hard requirement.
let _defineTheme = null;
let _generateThemeRulesSplit = null;
let _generateOnMediaCSS = null;
let _coreImportError = null;
try {
  const coreTheme = await import('@khameleon/core/theme');
...
function generatedHeader(sourceFile, lang = 'js', command) {
  ...
  const body = [
    `@generated by \`khameleon theme build\` — do not edit manually.`,
    `Source: ${sourceFile}`,
  ];
  if (command) {
...
/**
 * Resolve `@khameleon/core`'s package root relative to the CLI package. Core
 * and the CLI ship as siblings (`@khameleon/core`, `@khameleon/cli`), so
...
/**
 * Read the type declarations `@khameleon/core/<Component>` exposes so we can
...
/**
 * Determine whether `@khameleon/core/<Component>` exports an interface named
 ...
 *
 * An augmentation is only emitted when `@khameleon/core/<Component>` actually
...
async function generateVariantDeclarationsAsync(themeDef) {
  ...
  const sections = ['// Generated by khameleon theme build', 'export {};', ''];
  ...
  for (const [component, props] of Object.entries(customValues)) {
    for (const [prop, values] of Object.entries(props)) {
      ...
      const modulePath = `@khameleon/core/${pascal}`;
...
// Theme @scope selector helpers. Keep the `khameleon` literal in sync with
// packages/core/src/naming.ts (NAMESPACE) and generateThemeRules.ts.
// Theme scopes to data-khameleon-theme; the static build path must match.
const themeScopeStart = name => `[data-khameleon-theme="${name}"]`;
const THEME_SCOPE_TO = `[data-khameleon-theme]`;
...
function generateBuiltModule(themeDef, iconInfo) {
  ...
  return `${iconImport}/**
 * ${themeDef.name} theme — built by \`${getRunPrefix()} khameleon theme build\`
...
`
    : '';
  return `${variantsRef}import type { DefinedTheme } from '@khameleon/core/theme';
...
/**
 * Known Khameleon component names and their visual props.
...
/**
 * Path to this CLI's real entry (bin/khameleon.mjs), resolved from this module's
 * location (src/commands/build-theme.mjs → ../../bin/khameleon.mjs). Used to
 * re-invoke `theme build` as a child process in watch mode.
 */
function resolveCliBin() {
  const commandsDir = path.dirname(fileURLToPath(import.meta.url));
  return path.resolve(commandsDir, '../../bin/khameleon.mjs');
}
...
export function registerTheme(program) {
  const theme = program
    ...
    .action((options, cmd) => {
      ...
      // unknown subcommand (e.g. `khameleon theme bogus`), Commander hands it to
      ...
      // Bare `khameleon theme` — show the subcommand list. Exit 0 (help is success).
      theme.help();
    });
  ...
  theme
    ...
    .action(async (file, options) => {
      ...
      // `khameleon theme build` and the `<Theme>` runtime MUST emit identical CSS, so
      // there is exactly one generation path: @khameleon/core/theme. If core could not
      // be imported, fail hard rather than silently producing divergent output.
      if (!_defineTheme || !_generateThemeRulesSplit) {
        cliError(
          'Could not load @khameleon/core/theme — `khameleon theme build` requires a ' +
            'built, resolvable @khameleon/core so it emits the same CSS as the ' +
            'runtime <Theme>. Build @khameleon/core first (e.g. `pnpm -F @khameleon/core ' +
            'build`)' +
      ...
      let resolvedTheme;
      {
        ...
        if (_generateOnMediaCSS) {
          ...
          if (onMediaCss) {
            cssParts.push(`@layer khameleon-theme {\n${onMediaCss}\n}`);
          }
        }
      ...
      const buildCommand = `khameleon theme build ${sourceRelative}${options.out ? ' --out ' + path.relative(process.cwd(), path.resolve(process.cwd(), options.out)) : ''}`;
...
`);
  ...
  theme
    ...
    .action(async () => {
      ...
      humanLog('\nUsage:');
      humanLog('  khameleon theme add <slug> [target-path]   Scaffold a theme file you own\n');
    });
