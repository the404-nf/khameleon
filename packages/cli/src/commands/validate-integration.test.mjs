...
/**
 * @file Command-level tests for `khameleon validate-integration` — envelope shape
...
beforeEach(() => {
  ...
  tmpDir = fs.mkdtempSync(path.join(process.cwd(), '.khameleon-validate-cmd-'));
  logCalls = [];
...
describe('validate-integration — command', () => {
  it('--json emits an integration.validate envelope for a valid package', async () => {
    ...
    fs.writeFileSync(
      path.join(tmpDir, 'khameleon.integration.mjs'),
      'export default {};\n',
    );
    process.chdir(tmpDir);

    const program = createProgram();
    await program.parseAsync(['node', 'khameleon', '--json', 'validate-integration']);
  ...
  it('no-arg + no manifest prints guidance and stays exit 0', async () => {
    ...
    const program = createProgram();
    await program.parseAsync(['node', 'khameleon', 'validate-integration']);

    const out = logCalls.join('\n');
    expect(out).toContain('No khameleon.integration.* found next to package.json');
    expect(process.exitCode).toBeUndefined();
  });
