...
/**
 * @file Regression test for prose output of `khameleon theme build`.
 *
 * `khameleon theme build` has a single CSS-generation path — @khameleon/core's generator,
 ...
 *     `@layer khameleon-theme`, so component/Markdown StyleX always wins;
...
let tmpDir;
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'khameleon-build-theme-prose-'));
});
afterEach(() => {
...
describe('theme build prose output', () => {
  it('always emits prose mappings in @layer reset', () => {
    ...
    // NOT @layer khameleon-theme, so Markdown/component StyleX always wins.
