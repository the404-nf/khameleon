...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'khameleon-doctor-test-'));
  logCalls = [];
  exitCode = undefined;
...
afterEach(() => {
  ...
  vi.restoreAllMocks();
  delete process.env.KHAMELEON_THEME;
});

/** Make a minimal node_modules/@khameleon/core in tmpDir with the given version. */
function installCore(version = '0.0.14', peerDependencies) {
  const coreDir = path.join(tmpDir, 'node_modules', '@khameleon', 'core');
  fs.mkdirSync(coreDir, {recursive: true});
  const pkg = {name: '@khameleon/core', version};
...
describe('doctor — individual checks', () => {
  ...
  it('core-installed: FAIL when @khameleon/core is missing, PASS when present', () => {
  ...
  it('themes: WARN when theme installed but not wired', () => {
    installPkg('@khameleon/theme-neutral', '0.0.14');
    const res = checkThemes({cwd: tmpDir, configTheme: null});
    expect(res.status).toBe('warn');
    expect(res.message).toContain('@khameleon/theme-neutral');
  });
  ...
  it('themes: detects pnpm-style symlinked theme packages (#3530)', () => {
    installPkgPnpmStyle('@khameleon/theme-neutral', '0.1.2');
    const res = checkThemes({cwd: tmpDir, configTheme: 'default'});
    expect(res.status).toBe('pass');
    expect(res.message).toContain('@khameleon/theme-neutral');
  });

  it('config: INFO when no khameleon.config.mjs', async () => {
    const res = await checkConfig({cwd: tmpDir, configPath: null});
    expect(res.status).toBe('info');
  });

  it('config: PASS when a valid khameleon.config.mjs loads', async () => {
    ...
    const configPath = path.join(fixtureDir, 'khameleon.config.mjs');
    try {
  ...
  it('config: FAIL when khameleon.config.mjs throws on import', async () => {
    const fixtureDir = fs.mkdtempSync(
      path.join(path.dirname(fileURLToPath(import.meta.url)), '__doctor_cfg_'),
    );
    const configPath = path.join(fixtureDir, 'khameleon.config.mjs');
  ...
  it('agent-docs: INFO when no docs present', () => {
    ...
    expect(res.status).toBe('info');
    expect(res.fix).toContain('khameleon init');
  });
...
describe('doctor — command', () => {
  it('--json emits a doctor envelope', async () => {
    ...
    await program.parseAsync(['node', 'khameleon', '--json', 'doctor']);
  ...
  it('exit code stays 0 when there are no failures', async () => {
    // Run against the monorepo (where @khameleon/core resolves) → no FAIL.
    const prevExit = process.exitCode;
    process.exitCode = undefined;
    const program = createProgram();
    await program.parseAsync(['node', 'khameleon', 'doctor']);
    // In the repo, core resolves, so no failures → exitCode untouched.
    expect(process.exitCode).toBeUndefined();
    process.exitCode = prevExit;
  });

  it('sets exit code 1 when a check FAILs (bare dir, no @khameleon/core)', async () => {
