...
/**
 * @file Khameleon CLI — Commander program setup
 *
...
program
  ...
  .action((options, cmd) => {
    ...
    if (program.opts().json) {
      ...
      // standalone via `khameleon manifest --json` as `type: 'manifest'`).
      process.__xdsJsonHandled = true;
...
// scraping `--help`. `khameleon manifest --json` is the dedicated surface; the bare
...
program
  ...
  .action(() => {
    ...
    console.log(`\nRun \`khameleon manifest --json\` for the full structured manifest.\n`);
  });
