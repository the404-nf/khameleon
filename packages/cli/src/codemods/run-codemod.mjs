 ...
 * khameleon.config.* file; code codemods are applied to source files discovered
...
/**
 * Apply a config codemod to the consumer's khameleon.config.* file.
 *
...
export function runConfigCodemod(entry, {apply, log, jscodeshift}) {
  ...
  if (!configPath) {
    log.info(`  ${codemod.title} — no khameleon.config.* found; skipping.`);
