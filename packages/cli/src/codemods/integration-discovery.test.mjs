...
/**
 * Build a consumer project with an khameleon.config.mjs that loads a single
...
beforeEach(() => {
  ...
  tmpDir = fs.mkdtempSync(path.join(process.cwd(), '.khameleon-codemod-test-'));
...
describe('integration codemod discovery', () => {
  ...
  it('discovers and runs a config codemod against khameleon.config.*', async () => {
    scaffold({
    ...
    expect(result.totalFilesChanged).toBe(1);
    expect(
      fs.readFileSync(path.join(tmpDir, 'khameleon.config.mjs'), 'utf-8'),
