 ...
 * codemod). A config codemod runs against the consumer's khameleon.config.* file
...
export async function runCodemods(
  ...
  // Config codemods target the consumer's khameleon.config.* and never read
  ...
  for (const {version, transforms} of versionManifests) {
    ...
    for (const transformEntry of transforms) {
      ...
      // `(file, api)` contract and targets the consumer's khameleon.config.*.
  ...
  if (skippedOptional.length > 0) {
    ...
    for (const {name, meta} of skippedOptional) {
      ...
      log.info(
        `    Run: khameleon upgrade --codemod ${name} --path <dir> --apply`,
      );
    }
  }
