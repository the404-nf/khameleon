...
beforeEach(() => {
  ...
  tmpDir = fs.mkdtempSync(path.join(process.cwd(), '.khameleon-runner-test-'));
...
describe('runCodemods — unified config codemod path', () => {
  it('routes a core config codemod through the (file, api) runner to edit khameleon.config.*', async () => {
    fs.writeFileSync(
