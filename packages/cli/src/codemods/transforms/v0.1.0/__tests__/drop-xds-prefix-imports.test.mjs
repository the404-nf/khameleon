...
describe('drop-xds-prefix-imports', () => {
  ...
  it('aliases to Khameleon<Name> when the bare name collides with a local export function', async () => {
    const input = [
    ...
    const output = await applyTransform(input);
    // Import aliased to KhameleonCodeBlock; local declaration untouched.
    expect(output).toContain('CodeBlock as KhameleonCodeBlock');
    expect(output).toContain('@xds/core/CodeBlock');
    expect(output).toContain('export function CodeBlock({code}');
    expect(output).toContain('<KhameleonCodeBlock code={code} />');
  ...
  it('aliases on collision with a local const/class binding', async () => {
    ...
    const output = await applyTransform(input);
    expect(output).toContain('import {Card as KhameleonCard}');
    expect(output).toContain('const Card = 42;');
    expect(output).toContain('<KhameleonCard />');
  ...
  it('un-prefixes normally when there is NO local collision', async () => {
    ...
    expect(output).not.toContain('KhameleonCodeBlock');
  });
});
