...
describe('migrate-xds-css-surfaces', () => {
  it('rewrites the .xds- class-selector prefix', async () => {
    ...
    expect(output).toBe('.khameleon-heading { color: red; }');
  });

  it('rewrites the [data-xds-theme] attribute selector', async () => {
    const input = '[data-xds-theme="dark"] .xds-card { background: black; }';
    const output = await applyTransform(input);
    expect(output).toContain('[data-khameleon-theme="dark"]');
    expect(output).toContain('.khameleon-card');
  ...
  it('rewrites data-xds-theme-prose and data-xds-media attribute selectors', async () => {
    ...
    const output = await applyTransform(input);
    expect(output).toContain('[data-khameleon-theme-prose]');
    expect(output).toContain('[data-khameleon-media="print"]');
  });

  it('rewrites @layer xds-theme and @layer xds-base', async () => {
    const input = '@layer xds-theme, xds-base;\n@layer xds-theme { a { color: red; } }';
    const output = await applyTransform(input);
    expect(output).toContain('khameleon-theme');
    expect(output).toContain('khameleon-base');
  ...
  it('leaves already-migrated CSS unchanged', async () => {
    const input =
      '@layer khameleon-theme { [data-khameleon-theme="dark"] .khameleon-card {} }';
    const output = await applyTransform(input);
    expect(output).toBe(input);
  });
  ...
  it('rewrites @import of @xds/* package stylesheets', async () => {
    ...
    const output = await applyTransform(input);
    expect(output).toContain(`@import '@khameleon/core/reset.css';`);
    // xds.css is renamed to khameleon.css (file rename, not just scope swap).
    expect(output).toContain(`@import '@khameleon/core/khameleon.css';`);
  ...
  it('collapses theme-daily @import to theme-neutral and preserves quote style', async () => {
    ...
    const output = await applyTransform(input);
    expect(output).toBe(`@import "@khameleon/theme-neutral/theme.css";`);
  });

  it('rewrites @import url() form', async () => {
    const input = `@import url('@xds/core/reset.css');`;
    const output = await applyTransform(input);
    expect(output).toContain(`@khameleon/core/reset.css`);
    expect(output).not.toContain('@xds/');
  });
