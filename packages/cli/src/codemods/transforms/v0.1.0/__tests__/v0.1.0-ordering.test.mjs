 ...
 * `@khameleon/core` in a single upgrade pass.
 */
...
describe('v0.1.0 codemod manifest ordering', () => {
  ...
  it('un-prefixes useXDSTheme from @xds/core/theme AND renames the scope to @khameleon', async () => {
    const input = [
    ...
    expect(output).toMatch(/import\s*\{useTheme\}\s*from\s*['"]@khameleon\/core\/theme['"]/);
  ...
  it('un-prefixes XDSButton and XDSIconRegistry alongside the scope rename', async () => {
    ...
    expect(output).toMatch(/from\s*['"]@khameleon\/core['"]/);
