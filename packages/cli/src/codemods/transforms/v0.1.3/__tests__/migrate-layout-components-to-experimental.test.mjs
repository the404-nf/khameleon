 ...
 */
async function applyTransform(source, filePath = 'khameleon.config.mjs') {
...
`;
  ...
  it('handles createConfig({ ... }) wrapper', async () => {
    const input = `import { createConfig } from '@khameleon/cli';
...
describe('migrate-layout-components-to-experimental — end-to-end via runConfigCodemod', () => {
  it('rewrites khameleon.config.mjs on disk through the unified runner', async () => {
    ...
    const tmpDir = fs.mkdtempSync(
      path.join(process.cwd(), '.khameleon-v013-codemod-test-'),
    );
    try {
