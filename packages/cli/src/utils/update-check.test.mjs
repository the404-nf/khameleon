...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'khameleon-update-check-'));
  // Save and clear env
  originalEnv.KHAMELEON_LATEST_VERSION = process.env.KHAMELEON_LATEST_VERSION;
  delete process.env.KHAMELEON_LATEST_VERSION;
});

afterEach(() => {
  fs.rmSync(tmpDir, {recursive: true, force: true});
  // Restore env
  if (originalEnv.KHAMELEON_LATEST_VERSION !== undefined) {
    process.env.KHAMELEON_LATEST_VERSION = originalEnv.KHAMELEON_LATEST_VERSION;
  } else {
    delete process.env.KHAMELEON_LATEST_VERSION;
  }
});
...
describe('getLatestVersion', () => {
  it('reads from KHAMELEON_LATEST_VERSION env var', () => {
    process.env.KHAMELEON_LATEST_VERSION = '0.0.8';
    expect(getLatestVersion(tmpDir)).toBe('0.0.8');
  });

  it('ignores invalid env var values', () => {
    process.env.KHAMELEON_LATEST_VERSION = 'not-a-version';
...
describe('getInstalledVersion', () => {
  it('reads @khameleon/core version from dependencies', () => {
    fs.writeFileSync(
      path.join(tmpDir, 'package.json'),
      JSON.stringify({dependencies: {'@khameleon/core': '^0.0.7'}}),
    );
  ...
  it('strips semver range prefix', () => {
    fs.writeFileSync(
      path.join(tmpDir, 'package.json'),
      JSON.stringify({dependencies: {'@khameleon/core': '~0.0.5'}}),
    );
    expect(getInstalledVersion(tmpDir)).toBe('0.0.5');
  });

  it('returns null when @khameleon/core is not a dependency', () => {
    fs.writeFileSync(
      path.join(tmpDir, 'package.json'),
...
describe('checkForUpdate', () => {
  it('returns hint when newer version is available', () => {
    ...
    expect(hint).toContain('khameleon upgrade --from <old-version> --apply');
    expect(hint).toContain('FYI');
  });

  it('returns null when up to date', () => {
    process.env.KHAMELEON_LATEST_VERSION = '0.0.7';
    fs.writeFileSync(
      path.join(tmpDir, 'package.json'),
      JSON.stringify({dependencies: {'@khameleon/core': '^0.0.7'}}),
    );
  ...
  it('uses semver (not lexicographic) comparison for double-digit patches', () => {
    ...
    // 0.0.20 is available.
    process.env.KHAMELEON_LATEST_VERSION = '0.0.20';
    fs.writeFileSync(
      path.join(tmpDir, 'package.json'),
      JSON.stringify({dependencies: {'@khameleon/core': '^0.0.5'}}),
    );
  ...
  it('does not suggest an upgrade when latest < installed (semver)', () => {
    ...
    process.env.KHAMELEON_LATEST_VERSION = '0.0.9';
    fs.writeFileSync(
      path.join(tmpDir, 'package.json'),
      JSON.stringify({dependencies: {'@khameleon/core': '^0.0.10'}}),
    );
