...
function makeTmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'khameleon-paths-test-'));
}
...
describe('findCoreDir', () => {
  ...
  it('finds node_modules/@khameleon/core fallback', () => {
    const nmCore = path.join(tmpDir, 'node_modules', '@khameleon', 'core');
...
describe('discoverExternalPackages', () => {
  it('finds packages with an "khameleon" field in package.json', () => {
    const nm = path.join(tmpDir, 'node_modules');
    const extDir = path.join(nm, 'khameleon-charts');
    fs.mkdirSync(extDir, {recursive: true});
    fs.writeFileSync(
      path.join(extDir, 'package.json'),
      JSON.stringify({
        name: 'khameleon-charts',
        khameleon: {docs: './src', category: 'Data Viz'},
      }),
    );
  ...
  it('handles scoped packages (@org/pkg)', () => {
    const nm = path.join(tmpDir, 'node_modules');
    const scopedDir = path.join(nm, '@acme', 'khameleon-widgets');
    fs.mkdirSync(scopedDir, {recursive: true});
    fs.writeFileSync(
      path.join(scopedDir, 'package.json'),
      JSON.stringify({
        name: '@acme/khameleon-widgets',
        khameleon: {docs: './lib', category: 'Widgets'},
      }),
    );
  ...
  it('skips @khameleon/core', () => {
    const nm = path.join(tmpDir, 'node_modules');
    const coreDir = path.join(nm, '@khameleon', 'core');
    fs.mkdirSync(coreDir, {recursive: true});
    fs.writeFileSync(
  ...
  it('skips packages without "khameleon" field', () => {
    const nm = path.join(tmpDir, 'node_modules');
  ...
  it('walks up directories to find node_modules', () => {
    const nm = path.join(tmpDir, 'node_modules');
    const extDir = path.join(nm, 'khameleon-ext');
    fs.mkdirSync(extDir, {recursive: true});
    fs.writeFileSync(
      path.join(extDir, 'package.json'),
      JSON.stringify({
    ...
    expect(result[0].name).toBe('khameleon-ext');
  });
