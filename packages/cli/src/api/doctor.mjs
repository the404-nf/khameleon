...
/**
 * @file Health-check engine for `khameleon doctor`.
 *
 ...
 * @property {string|null} coreDir - Resolved @khameleon/core directory, or null.
 * @property {string|null} configPath - Resolved khameleon.config.mjs path, or null.
...
/**
 * Find every installed @khameleon/theme-* package under node_modules.
 * @param {string} cwd
...
function findThemePackages(cwd) {
  ...
  const found = [];
  if (!nm) return found;
  const scopeDir = path.join(nm, '@khameleon');
  ...
  for (const entry of entries) {
    ...
    if (!isDir) continue;
    const name = `@khameleon/${entry.name}`;
...
/**
 * Detect whether a theme appears to be wired up via the KHAMELEON_THEME env var or
...
function detectThemeWiring(cwd) {
  if (process.env.KHAMELEON_THEME) return {wired: true, source: 'KHAMELEON_THEME env var'};
  const nm = findNodeModules(cwd);
  const projectDir = nm ? path.dirname(nm) : cwd;
  const pkg = readPkg(path.join(projectDir, 'package.json'));
  if (pkg?.khameleon?.theme) return {wired: true, source: 'package.json khameleon.theme'};
...
/**
 * Check 2 — @khameleon/core is installed and resolvable from the project.
 * @param {DoctorContext} ctx
 * @returns {DoctorCheck}
 */
export function checkCoreInstalled(ctx) {
  ...
  return {
    id: 'core-installed',
    label: '@khameleon/core installed',
    status: found ? 'pass' : 'fail',
    message: found
      ? `@khameleon/core resolved${version ? ` (v${version})` : ''}.`
      : '@khameleon/core could not be resolved from this project.',
    ...(found
      ? {}
      : {fix: 'Install the design system: `npm install @khameleon/core` (or yarn/pnpm/bun).'}),
  };
}

/**
 * Check 3 — installed @khameleon/core is in step with @khameleon/cli (major/minor drift).
 * @param {DoctorContext} ctx
 * @returns {DoctorCheck}
 */
export function checkVersionAlignment(ctx) {
  ...
  if (!coreVersion || !cliVersion) {
    return {
      ...
      label: '@khameleon/core <-> @khameleon/cli alignment',
      status: 'info',
      message: 'Skipped — could not read both @khameleon/core and @khameleon/cli versions.',
    };
  }
  ...
  return {
    ...
    message: drift
      ? `@khameleon/core v${coreVersion} drifts from @khameleon/cli v${cliVersion} (major/minor mismatch).`
      : `@khameleon/core v${coreVersion} is in step with @khameleon/cli v${cliVersion}.`,
    ...(drift
      ? {
          fix:
            semverCompare(cliVersion, coreVersion) > 0
              ? `Update @khameleon/core to ${cliMajor}.${cliMinor}.x to match the CLI.`
              : `Update @khameleon/cli to ${coreMajor}.${coreMinor}.x to match @khameleon/core.`,
        }
      : {}),
  };
}
...
/**
 * Check 4 — at least one @khameleon/theme-* is installed and a theme is wired.
 * @param {DoctorContext} ctx
 * @returns {DoctorCheck}
 */
export function checkThemes(ctx) {
  ...
  if (themes.length === 0) {
    return {
      ...
      status: 'warn',
      message: 'No @khameleon/theme-* packages are installed.',
  ...
  if (!wired) {
    return {
      ...
      fix: 'Wire a theme via the `khameleon.theme` field in package.json, the KHAMELEON_THEME env var, or your khameleon.config.mjs.',
    };
  }

  const source = hasConfigTheme ? 'khameleon.config.mjs theme' : wiring.source;
  return {
    id: 'themes',
    label: 'Theme packages',
    status: 'pass',
...
/**
 * Check 5 — khameleon.config.mjs (if present) loads and has a valid shape.
 * @param {DoctorContext} ctx
...
export async function checkConfig(ctx) {
  if (!ctx.configPath) {
    return {
      id: 'config',
      label: 'khameleon.config.mjs',
      status: 'info',
      message: 'No khameleon.config.mjs found — using defaults.',
    };
  }
  ...
  try {
    ...
    if (config !== undefined && (typeof config !== 'object' || config === null)) {
      return {
        ...
        label: 'khameleon.config.mjs',
        status: 'fail',
        message: `khameleon.config.mjs default export is not an object (got ${typeof config}).`,
        fix: 'Export a default object from khameleon.config.mjs, e.g. `export default { integrations: [] };`.',
      };
    }
    return {
      id: 'config',
      label: 'khameleon.config.mjs',
      status: 'pass',
      message: `khameleon.config.mjs loaded cleanly (${path.relative(ctx.cwd, ctx.configPath) || ctx.configPath}).`,
    };
  } catch (err) {
    return {
      id: 'config',
      label: 'khameleon.config.mjs',
      status: 'fail',
      message: `khameleon.config.mjs failed to load: ${err.message}`,
      fix: 'Fix the syntax/runtime error in khameleon.config.mjs so it imports cleanly.',
    };
  }
}

/**
 * Check 6 — agent docs exist and contain the Khameleon section markers.
 * @param {DoctorContext} ctx
 * @returns {DoctorCheck}
 */
export function checkAgentDocs(ctx) {
  ...
  const withMarkers = present.filter(rel => {
    try {
      ...
      return (
        (content.includes('<!-- KHAMELEON:START -->') || content.includes('<!-- XDS:START -->')) &&
        (content.includes('<!-- KHAMELEON:END -->') || content.includes('<!-- XDS:END -->'))
      );
    } catch {
      return false;
    }
  ...
  if (withMarkers.length === 0) {
    return {
      ...
      label: 'AI agent docs',
      status: 'warn',
      message: `Agent docs present (${present.join(', ')}) but no Khameleon section markers found.`,
      fix: 'Add the Khameleon section to your agent docs with `khameleon init --features agents`.',
    };
  }
  ...
  return {
    ...
    label: 'AI agent docs',
    status: 'pass',
    message: `Khameleon agent docs section present in ${withMarkers.join(', ')}.`,
  };
}

/**
 * Check 7 — @khameleon/core peer dependencies are satisfied by installed packages.
 * @param {DoctorContext} ctx
 * @returns {DoctorCheck}
 */
export function checkPeerDeps(ctx) {
  if (!ctx.coreDir) {
    return {
      id: 'peer-deps',
      label: '@khameleon/core peer dependencies',
      status: 'info',
      message: 'Skipped — @khameleon/core is not installed.',
    };
  }
  ...
  if (peerNames.length === 0) {
    return {
      ...
      label: '@khameleon/core peer dependencies',
      status: 'info',
      message: '@khameleon/core declares no peer dependencies.',
    };
  }
