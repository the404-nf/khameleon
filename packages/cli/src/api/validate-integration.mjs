...
/**
 * @file Programmatic API for `khameleon validate-integration`.
 *
 * Validates exactly ONE integration package at a time and reports findings
 * using the KhameleonIntegrationIssue model
 ...
 *     package.json + sibling khameleon.integration.{ts,mjs,js}).
...
/**
 * @typedef {import('../types/integration').KhameleonIntegrationIssue} Issue
 */
...
async function validateAtPackageDir(packageDir, identity) {
  ...
  if (manifests.length === 0) {
    issues.push(
      error(
        'missing_manifest',
        `No khameleon.integration.{ts,mjs,js} found next to package.json in ${packageDir}.`,
      ),
    );
    return result;
  }
 ...
 * + a single sibling khameleon.integration.{ts,mjs,js}. A missing manifest yields
