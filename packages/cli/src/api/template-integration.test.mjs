 ...
 *
 * These tests stand up a temp consumer project with an khameleon.config and an
...
function makeConsumer() {
  const dir = fs.mkdtempSync(
    path.join(process.cwd(), '.khameleon-template-it-'),
  );
  fs.writeFileSync(
  ...
  fs.writeFileSync(
    path.join(dir, 'khameleon.config.mjs'),
...
function installWidgets(consumerDir) {
  ...
  fs.writeFileSync(
    path.join(pkgDir, 'khameleon.integration.mjs'),
...
describe('integration template discovery', () => {
  ...
  it('always reports core templates under @khameleon/core', async () => {
    const result = await template(undefined, {list: true, cwd: tmpDir});
    const core = result.data.filter(t => t.package === '@khameleon/core');
  ...
  it('errors with candidates when an id is ambiguous across type/package', async () => {
    ...
    fs.writeFileSync(
      path.join(pkg2, 'khameleon.integration.mjs'),
      `export default { templates: './templates' };\n`,
    );
    fs.writeFileSync(
