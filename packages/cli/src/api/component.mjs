 ...
 *
 * Returns the same typed envelope { type, data } that `khameleon --json component` outputs.
...
export async function component(name, options = {}) {
  ...
  if (!coreDir) {
    throw new KhameleonError('Could not find @khameleon/core package', undefined, ERROR_CODES.ERR_CORE_NOT_FOUND);
  }
  ...
  if (category || list || !name) {
    ...
    // Back-compat: node_modules-scanned external packages (pkg.khameleon.docs)
  ...
  if (packageScope) {
    ...
    if (packageScope === CORE_PACKAGE) {
      ...
      if (!owner) {
        throw new KhameleonError(`No component "${name}" in package "${packageScope}"`, undefined, ERROR_CODES.ERR_UNKNOWN_COMPONENT);
      }
      if (source) {
        if (!owner.sourcePath) {
          throw new KhameleonError(`Source for "${name}" not found`, undefined, ERROR_CODES.ERR_NO_SOURCE);
        }
    ...
    if (integration) {
      ...
      if (source) {
        if (!owner.sourcePath) {
          throw new KhameleonError(`Source for "${name}" not found in package "${packageScope}"`, undefined, ERROR_CODES.ERR_NO_SOURCE);
        }
        return {type: 'component.detail.source', data: {component: dirName, source: fs.readFileSync(owner.sourcePath, 'utf-8')}};
      }
      const docs = await loadDocs(owner.docPath, {zh, dense, lang});
    ...
    // Legacy fallback: node_modules `pkg.khameleon.docs` external package.
    const ext = resolveExternalPackage(packageScope, cwd);
    if (!ext) {
      throw new KhameleonError(`External package "${packageScope}" not found`, undefined, ERROR_CODES.ERR_UNKNOWN_PACKAGE);
    }

    if (showcase) {
      const match = await findShowcase(dirName, cwd, {package: packageScope});
      if (!match) {
        throw new KhameleonError(`No showcase found for "${name}" in package "${packageScope}"`, undefined, ERROR_CODES.ERR_NO_SHOWCASE);
      }
      return {
  ...
  // NOTE: legacy `pkg.khameleon.docs` externals are intentionally NOT part of this
  ...
  if (owners.length === 1 && owners[0].package !== CORE_PACKAGE) {
    ...
    if (showcase) {
      throw new KhameleonError(`No showcase found for "${name}"`, undefined, ERROR_CODES.ERR_NO_SHOWCASE);
    }
    const docs = await loadDocs(owner.docPath, {zh, dense, lang});
    if (props) {
      const p = docs.props || (docs.components ? docs.components.flatMap(c => c.props || []) : []);
  ...
  if (!readmePath) {
    ...
    if (results.length > 0) {
      ...
      throw new KhameleonError(`No component named "${name}"`, undefined, ERROR_CODES.ERR_UNKNOWN_COMPONENT);
    }
  }

  if (!readmePath || !readmePath.endsWith('.doc.mjs')) {
    throw new KhameleonError(`No .doc.mjs found for "${resolvedName}". The component needs a typed doc file.`, undefined, ERROR_CODES.ERR_NO_DOC);
  }
