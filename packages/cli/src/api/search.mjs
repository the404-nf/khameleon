...
async function gatherHooks(coreDir) {
  ...
  for (const hookName of names) {
    ...
    let keywords = [];
    let description = '';
    let importPath = '@khameleon/core/hooks';
    if (docPath) {
  ...
  switch (c.domain) {
    case 'component':
      return {
        ...base,
        import: c._import,
        command: `khameleon component ${c.name}`,
      };
    case 'hook':
      return {
        ...base,
        import: c._import,
        command: `khameleon hook ${c.name}`,
      };
    case 'doc':
      return {
        ...base,
        title: c._title,
        command: `khameleon docs ${c.name}`,
      };
    case 'template':
      return {
        ...base,
        ...
        kind: c._kind,
        command: `khameleon template ${c.name}`,
      };
    default:
      return base;
  }
...
export async function search(query, options = {}) {
  ...
  if (!query || !String(query).trim()) {
    throw new KhameleonError('A search query is required', [
      {name: 'khameleon search button', reason: 'example'},
    ]);
  }
  ...
  if (!coreDir) {
    throw new KhameleonError('Could not find @khameleon/core package');
  }
