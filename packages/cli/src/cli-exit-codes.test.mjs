...
describe('exit codes — error paths (--json → exit 1, valid envelope)', () => {
  it('khameleon bogus-cmd --json → exit 1, error envelope', () => {
  ...
  it('khameleon component bogus --json → exit 1, error envelope', () => {
  ...
  it('khameleon hook bogus --json → exit 1, error envelope', () => {
  ...
  it('khameleon docs bogus --json → exit 1, error envelope', () => {
...
describe('exit codes — success paths (exit 0)', () => {
  it('khameleon (no args) → exit 0, prints help', () => {
    const r = expectExit([], 0);
    // Help should land on stdout (Commander default for help()).
    expect(r.stdout + r.stderr).toMatch(/Usage:/);
  });

  it('khameleon --help → exit 0', () => {
    expectExit(['--help'], 0);
  });

  it('khameleon --version → exit 0, prints a version', () => {
    const r = expectExit(['--version'], 0);
    expect(r.stdout.trim()).toMatch(/^\d+\.\d+\.\d+/);
  });

  it('khameleon component (list mode) → exit 0', () => {
    expectExit(['component'], 0);
  });

  it('khameleon component Button → exit 0', () => {
    expectExit(['component', 'Button'], 0);
  });

  it('khameleon docs (list mode) → exit 0', () => {
    expectExit(['docs'], 0);
  });

  it('khameleon theme (no subcommand) → exit 0, shows help', () => {
    // Bare theme without a subcommand is a help-display case → exit 0.
    expectExit(['theme'], 0);
  });

  it('khameleon hook (list mode) → exit 0', () => {
    expectExit(['hook'], 0);
  });
});
