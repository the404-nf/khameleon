 ...
 * `khameleon upgrade`, and otherwise return the definition unchanged. They do NOT
 ...
 *
 * @template {import('./types/codemod').KhameleonCodemodDef} T
 * @param {T} def
 * @returns {import('./types/codemod').KhameleonCodemod}
 */
export function createCodemod(def) {
  return /** @type {import('./types/codemod').KhameleonCodemod} */ ({
    ...def,
    type: 'code',
  });
}
...
/**
 * Define a codemod that targets the consumer's khameleon.config.* file. Stamp-only:
 * returns the definition with `type: 'config'` injected. Validation happens at
 * the load boundary.
 *
 * @template {import('./types/codemod').KhameleonConfigCodemodDef} T
 * @param {T} def
 * @returns {import('./types/codemod').KhameleonConfigCodemod}
 */
export function createConfigCodemod(def) {
  return /** @type {import('./types/codemod').KhameleonConfigCodemod} */ ({
    ...def,
    type: 'config',
  });
}
