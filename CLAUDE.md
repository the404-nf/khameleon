...
Tests how well AGENTS.md helps LLMs generate correct Khameleon component code.
...
1. Run `pnpm -F @khameleon/vibe-tests interactive --sample <count>` to set up iteration
2. Spawn parallel subagents (one per test prompt) to:
   - Read the task file from `results/<iteration>/tasks/{promptId}.json`
   - Generate code for the prompt using Khameleon components (AGENTS.md auto-injected)
...
4. Run `pnpm -F @khameleon/vibe-tests aggregate --iteration <id>` to see results
...
For architectural context, decisions, and research, see the **[GitHub Wiki](https://github.com/the404-nf/khameleon/wiki)**:
...
Khameleon CLI|Run from repo root. Load agent docs before any component work.
KHAMELEON="node packages/cli/bin/khameleon.mjs"
BOOTSTRAP (run every branch, <500ms):
$KHAMELEON help # discover all commands and options
$KHAMELEON docs # list available doc topics
$KHAMELEON docs principles --dense # design rules, anti-patterns, xstyle, tokens
$KHAMELEON docs tokens --dense # spacing, color, radius, typography, shadow
$KHAMELEON docs theme --dense # theme provider, light/dark, overrides
$KHAMELEON component --list # all components grouped by category
$KHAMELEON template --list # available page templates
ON DEMAND:
$KHAMELEON component <Name> --dense # props, variants, usage, anatomy for one component
$KHAMELEON template <name> # emit full page source
$KHAMELEON template <name> --skeleton # layout skeleton with spatial annotations
$KHAMELEON swizzle <Name> # eject component source for deep customization
$KHAMELEON upgrade --apply # run version migration codemods
...
RULE: always run $KHAMELEON component <Name> --dense before modifying a component
RULE: after @khameleon/core bump, always run $KHAMELEON upgrade --apply
